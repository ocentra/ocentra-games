...
export function useGamesData(): GamesDataState {
  const initialSnapshot = cachedGamesDataSnapshot;
  ...
  useEffect(() => {
    let cancelled = false;
    const cachedSnapshot = refreshKey === 0 ? cachedGamesDataSnapshot : null;
    if (cachedSnapshot) {
    ...
    (async () => {
      try {
        ...
        const snapshot = await loadFreshGamesDataSnapshot();
        if (cancelled) return;
