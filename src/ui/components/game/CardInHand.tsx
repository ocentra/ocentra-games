...
// Hardcoded paths are deprecated. Use GUIDs or props for resource resolution.
const DEFAULT_CARD_BACK_PATH = ''; // Should be passed as a prop or resolved via GUID
...
const CardInHand: React.FC<CardInHandProps> = ({
  ...
  const normalizedCardImagePath = cardImage ? (cardImage.startsWith('/') ? cardImage : `/${cardImage}`) : null;
  const cardImageHash = normalizedCardImagePath && isImageHash(normalizedCardImagePath)
    ? normalizedCardImagePath
    : null;
  const { imageUrl: cardImageUrl } = useImageUrl(cardImageHash as ImageHash | null, {
    enabled: !!cardImageHash,
  });
  const resolvedCardImage = cardImageHash ? cardImageUrl : normalizedCardImagePath ?? cardImage;
    ...
    <div ref={containerRef} className="card-in-hand-container">
      {cards.map((_, index: number) => (
        <img
          ...
          src={resolvedCardImage || undefined}
          alt=""
