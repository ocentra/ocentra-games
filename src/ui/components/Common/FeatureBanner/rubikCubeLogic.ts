...
export function createRubikCubeController(config: RubikConfig = {}): RubikCubeController {
  ...
  return {
    setContainer(el: HTMLDivElement | null) {
      ...
      for (const options of rendererOptions) {
        try {
          ...
          if (LOG_RUBIK_VERBOSE) {
            log.logInfo('[RubikBannerCube] WebGL renderer created', getStackTrace(), {
              options,
          ...
          log.logWarn('[RubikBannerCube] WebGL renderer attempt failed', getStackTrace(), {
            options,
      ...
      const pixelRatio = isTauriRuntime()
      ...
      if (LOG_RUBIK_VERBOSE) {
        log.logInfo(
          '[RubikBannerCube] WebGL renderer created',
          getStackTrace(),
          {
            width: el.clientWidth,
            height: el.clientHeight,
            pixelRatio,
            estimatedFramebufferMiB: bytesToMiB(
              el.clientWidth * el.clientHeight * pixelRatio * pixelRatio * 4 * 2
            ),
    ...
    async setImages(sources: (HTMLCanvasElement | HTMLImageElement | string)[]) {
      ...
      if (LOG_RUBIK_VERBOSE) {
        log.logInfo(
          '[RubikBannerCube] Prepared direct face images',
          getStackTrace(),
          {
            sourceCount: sources.length,
            preparedImageCount: images.length,
            textureSize: size,
            estimatedTextureMiB: bytesToMiB(images.length * size * size * 4),
          }
        );
      }
    ...
    async setItemsForSplit(sources: (HTMLCanvasElement | HTMLImageElement | string)[]) {
      ...
      if (LOG_RUBIK_VERBOSE) {
        log.logInfo(
          '[RubikBannerCube] Prepared split face images',
          getStackTrace(),
          {
            sourceCount: sources.length,
            preparedFaceCount: images.length,
            textureSize: cellSize,
            estimatedTextureMiB: bytesToMiB(images.length * cellSize * cellSize * 4),
          }
        );
      }
    ...
    setConfig(partial: Partial<RubikConfig>) {
      if (partial.split != null) state.split = Math.max(2, Math.min(6, partial.split));
      ...
      if (partial.maxTextureSize != null) {
        state.maxTextureSize = Math.max(128, Math.min(512, partial.maxTextureSize));
      }
