...
import { ImageVariant } from '@ocentra/storage-domain/caches/ImageCacheService';
...
interface ImageCacheAdapter {
  getCachedImageByHash(hash: string, variant?: ImageVariant): Promise<{
    blob: Blob;
  } | null>;
}
...
export function useImageUrl(
  ...
  useEffect(() => {
    ...
    const checkCacheAndRetry = async () => {
      ...
      try {
        const imageCache: ImageCacheAdapter = getPlatformImageCacheAdapter();
        let cached = null;
