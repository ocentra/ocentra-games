...
export const loginWithGoogle = async (): Promise<AuthResult> => {
  ...
  const isCapacitor = typeof globalThis !== 'undefined' && (globalThis as { Capacitor?: unknown }).Capacitor;

  if (isTauri) {
    try {
      const { signInWithGoogleTauri } = await import('@/adapters/auth/GoogleOAuthTauri');
      const result = await signInWithGoogleTauri();
      logInfo('loginWithGoogle: ✅ Tauri native OAuth successful', { data: { uid: result.user.uid } }, LOG_AUTH_SOCIAL);
      ...
      if (!userDoc.exists()) {
        ...
        await updateDoc(doc(db!, FirestoreCollection.Users, result.user.uid), { lastLoginAt: new Date() });
        userProfile.lastLoginAt = new Date();
      }
      return { success: true, user: userProfile };
    } catch (error) {
      const msg = error instanceof Error ? error.message : 'Native Google sign-in failed';
      logError('loginWithGoogle: ❌ Tauri native OAuth failed', { data: { error: msg } }, LOG_AUTH_ERROR);
  ...
  if (isCapacitor) {
    try {
      ...
      if (!idToken) {
        logError('loginWithGoogle: ❌ Capacitor native Google Sign-In: no id_token in result', LOG_AUTH_ERROR);
        return { success: false, error: 'Google Sign-In failed: no credentials returned' };
      }
      const credential = GoogleAuthProvider.credential(idToken);
      const firebaseResult = await signInWithCredential(auth!, credential);
      logInfo('loginWithGoogle: ✅ Capacitor native Google Sign-In successful', { data: { uid: firebaseResult.user.uid } }, LOG_AUTH_SOCIAL);
      const userDoc = await getDoc(doc(db!, FirestoreCollection.Users, firebaseResult.user.uid));
      let userProfile: UserProfile;
      if (!userDoc.exists()) {
        userProfile = {
          uid: firebaseResult.user.uid,
          displayName: firebaseResult.user.displayName || firebaseResult.user.email?.split('@')[0] || 'Google User',
        ...
        await setDoc(doc(db!, FirestoreCollection.Users, firebaseResult.user.uid), userProfile);
      } else {
        userProfile = { ...userDoc.data() as UserProfile, uid: firebaseResult.user.uid };
        await updateDoc(doc(db!, FirestoreCollection.Users, firebaseResult.user.uid), { lastLoginAt: new Date() });
        userProfile.lastLoginAt = new Date();
      }
      return { success: true, user: userProfile };
    } catch (error) {
      const msg = error instanceof Error ? error.message : 'Native Google sign-in failed';
      logError('loginWithGoogle: ❌ Capacitor native Google Sign-In failed', { data: { error: msg } }, LOG_AUTH_ERROR);
      return { success: false, error: msg };
    }
  }
...
async function uploadAvatarToR2(dataUrl: string, userId: string): Promise<string | null> {
  try {
    ...
    if (!config.r2Assets?.enabled || !config.r2Assets?.workerUrl) {
