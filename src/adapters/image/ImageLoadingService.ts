...
import { ProcessingState, ImageVariant, type CachedImage } from '@ocentra/storage-domain/caches/ImageCacheService';
import { TIMEOUTS, CACHE_LIMITS } from '@ocentra/storage-domain/idb/idbConstants';
...
import { getPlatformImageCacheAdapter } from '@/adapters/image/PlatformImageCacheAdapter';
...
export class ImageLoadingService {
  ...
  private constructor() {
    this.imageCache = getPlatformImageCacheAdapter();
  ...
  private async handleBatchLoadRequest(event: ImageBatchLoadRequestEvent): Promise<void> {
    ...
    for (let i = 0; i < event.requests.length; i++) {
      ...
      try {
        if (request.variant === ImageVariant.Icon) {
          requestsToQueue.push({ hash, variant: request.variant, priority: request.priority });
          continue;
        }
        ...
        logError(`IndexedDB cache lookup failed: ${hash}`, { cacheError });
      }
