 ...
 * Per spec Section 12, line 584: "tsx scripts/solana/verification/upload-match.ts --file path/to/match.json --upload r2 --anchor solana:memo|program --batch-id <opt>"
...
async function uploadMatch(options: UploadOptions): Promise<void> {
  ...
  // Compute match hash (copy to ArrayBuffer for strict BufferSource typing)
  const hashBuffer = await crypto.subtle.digest(
    'SHA-256',
    new Uint8Array(canonicalBytes).buffer
  );
  ...
  if (options.anchor) {
    ...
    if (options.anchor === 'solana:program') {
      ...
      const gameWallet = {
        publicKey: keypair.publicKey,
        signTransaction: wallet.signTransaction.bind(wallet) as (_tx: unknown) => Promise<unknown>,
      };
