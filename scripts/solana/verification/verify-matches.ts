 ...
 * Note: Example files are in scripts/solana/verification/examples/
 */
function getExampleMatchIds(): string[] {
  const examplesDir = join(process.cwd(), 'scripts/solana/verification/examples');
  const matchIds: string[] = [];

  try {
    const humanVsHuman = JSON5.parse(
  ...
  try {
    ...
    if (!matchRecord && r2Service) {
      try {
        ...
        if (matchJson) {
          matchRecord = JSON5.parse(matchJson) as MatchRecord;
        }
      } catch (error) {
...
async function verifyAllMatches(): Promise<void> {
  ...
  const stubWallet = {
    ...
    signTransaction: async (tx: unknown) => {
      const transaction = tx as { sign: (_kp: Keypair) => void };
  ...
  if (process.env.R2_WORKER_URL) {
    r2Service = new R2Service({
      ...
      bucketName: process.env.R2_BUCKET_NAME || StorageBucketName.DefaultMatches,
    });
  }
