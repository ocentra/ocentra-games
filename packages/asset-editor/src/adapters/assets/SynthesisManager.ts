...
import { GameInfo, type ContentBlock } from '@ocentra/game-asset-domain/game/gameInfo/GameInfo';
...
export class SynthesisManager {
    ...
    private async handleSynthesizeGameInfo(event: SynthesizeGameInfoEvent): Promise<void> {
        ...
        try {
            ...
            const gameInfoSource = await ScriptableObject.loadByGuid(GameInfo, AssetGUID.from(gameInfoGuid));
            if (!gameInfoSource) {
    ...
    private async buildSynthesisContext(gameInfo: GameInfo): Promise<SynthesisContext> {
        ...
        try {
            ...
            const entriesDeferred = new OperationDeferred<AssetResourceEntry<GameMode>[]>();
            await EventBus.instance.publishAsync(new GetGameModeEntriesEvent(entriesDeferred));
            const entriesResult = await entriesDeferred.promise;

            if (entriesResult.isSuccess && entriesResult.value) {
                // Find entry that points to this gameInfo
                // This is a bit expensive but synthesis is an editor-time operation
                for (const entry of entriesResult.value) {
                    const gameMode = await ScriptableObject.loadByGuid(CardGameMode, AssetGUID.from(entry.guid));
                    if (gameMode && gameMode instanceof CardGameMode) {
                        // Check if it links to our gameInfo
                        // We might need to load it if it's not already loaded
                        await gameMode.readNestedAssetData([gameMode.gameInfoAsset]);
