...
export class CanonicalSerializer {
    static canonicalizeMatchRecord(match) {
    ...
    static normalizeMatchRecord(match) {
        ...
        normalized.moves = match.moves.map((m) => {
            const move = {
                ...
                timestamp: typeof m.timestamp === 'string' ? m.timestamp : this.toISO8601(m.timestamp),
