var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import 'reflect-metadata';
import { serializable, serializableClass } from '../serialization/decorators.js';
import { AssetTypeCategory } from '../constants/assets.js';
let ResourceEntry = class ResourceEntry {
    path = '';
    displayName = '';
    gameId;
    category;
    mimeType;
    fileSize;
    createdAt;
    updatedAt;
    lastScanAt;
    checksum;
};
__decorate([
    serializable({ label: 'Path' }),
    __metadata("design:type", String)
], ResourceEntry.prototype, "path", void 0);
__decorate([
    serializable({ label: 'Display Name' }),
    __metadata("design:type", String)
], ResourceEntry.prototype, "displayName", void 0);
__decorate([
    serializable({ label: 'Game ID' }),
    __metadata("design:type", Object)
], ResourceEntry.prototype, "gameId", void 0);
__decorate([
    serializable({ label: 'Category' }),
    __metadata("design:type", Object)
], ResourceEntry.prototype, "category", void 0);
__decorate([
    serializable({ label: 'MIME Type' }),
    __metadata("design:type", Object)
], ResourceEntry.prototype, "mimeType", void 0);
__decorate([
    serializable({ label: 'File Size' }),
    __metadata("design:type", Object)
], ResourceEntry.prototype, "fileSize", void 0);
__decorate([
    serializable({ label: 'Created At' }),
    __metadata("design:type", Object)
], ResourceEntry.prototype, "createdAt", void 0);
__decorate([
    serializable({ label: 'Updated At' }),
    __metadata("design:type", Object)
], ResourceEntry.prototype, "updatedAt", void 0);
__decorate([
    serializable({ label: 'Last Scan At' }),
    __metadata("design:type", Object)
], ResourceEntry.prototype, "lastScanAt", void 0);
__decorate([
    serializable({ label: 'Checksum' }),
    __metadata("design:type", Object)
], ResourceEntry.prototype, "checksum", void 0);
ResourceEntry = __decorate([
    serializableClass({
        assetType: 'ResourceEntry',
        displayName: 'Resource Entry',
        icon: '📄',
        category: AssetTypeCategory.Content,
    })
], ResourceEntry);
export { ResourceEntry };
