...
function buildJsonDescriptors() {
    ...
    for (const file of files) {
        ...
        if (also)
            parts.push('.', `Also: ${truncate(also, 120)}`);
...
function sourceFromFilename(file) {
    if (file.startsWith('pagat-'))
        return 'Pagat';
    if (file.startsWith('wiki-'))
        return 'Wikipedia';
    if (file.startsWith('britannica-'))
        return 'Britannica';
    if (file.startsWith('bicyclecards-'))
        return 'Bicycle Cards';
    if (file.startsWith('wizardofodds-'))
        return 'Wizard of Odds';
    if (file.startsWith('semicolon-'))
        return 'Semicolon';
    if (file.startsWith('parlett-'))
        return 'Parlett';
    if (file.startsWith('acecardgames-'))
        return 'Ace Card Games';
    if (file.startsWith('namu-'))
        return 'Namu Wiki';
    if (file.startsWith('riichi-wiki'))
        return 'Riichi Wiki';
    if (file.startsWith('catsatcards-'))
        return 'Cats at Cards';
    if (file.startsWith('bvssolitaire-'))
        return 'BVS Solitaire';
    if (file.startsWith('fismelab-'))
        return 'Fismelab';
    if (file.startsWith('onpixelgames-'))
        return 'OnPixelGames';
    if (file.startsWith('ispa-'))
        return 'ISPA';
    if (file.startsWith('norskespilleautomater-'))
        return 'Norskespilleautomater';
    if (file.startsWith('scribd-'))
        return 'Scribd';
    if (file.startsWith('cuttle-'))
        return 'Cuttle';
    if (file.startsWith('piatnik-'))
        return 'Piatnik';
    return 'Other';
}
