...
if (existsSync(MANIFEST)) {
    ...
    for (const [file, url] of Object.entries(m)) {
        ...
        if (!manifestUrls.has(n))
            manifestUrls.set(n, []);
        manifestUrls.get(n).push(file);
        if (!manifestNormToOriginal.has(n))
            manifestNormToOriginal.set(n, { file, url });
    }
    const redirects = (m._redirects || {});
...
function getUrlsFromJson(jsonFile) {
    try {
        ...
        const additional = data?.sources?.additional ?? [];
        for (const a of additional) {
            if (typeof a === "string" && /^https?:\/\//.test(a.trim()))
                out.push(a.trim());
            if (a && typeof a === "object" && typeof a.url === "string" && /^https?:\/\//.test(a.url.trim()))
                out.push(a.url.trim());
        }
        return out;
    }
...
function manifestUrlCovered(normUrl) {
...
function manifestHasUrl(normUrl) {
    if (manifestUrls.has(normUrl))
        return true;
    if (/pagat\.com/i.test(normUrl)) {
        const alt = normUrl.replace(/\/schafk\//, "/schafkopf/").replace(/\/schafkopf\//, "/schafk/");
        const alt2 = normUrl.replace(/\/quotawhist\//, "/whist/").replace(/\/whist\//, "/quotawhist/");
        if (manifestUrls.has(alt) || manifestUrls.has(alt2))
            return true;
    }
    if (/catsatcards\.com/i.test(normUrl)) {
        const alt = normUrl.replace(/^https:\/\/www\./, "https://").replace(/^https:\/\/(?!www\.)/, "https://www.");
        if (manifestUrls.has(alt))
            return true;
    }
    return false;
}
const jsonUrlsWithoutHtml = [];
...
const indexLike = /\/index\.html?$|\/(com|domino|fishing|banking|allfours|adders|beating|auctionwhist|boston|rams|patience|climbing|tarot)\/$|\/line\/$|\/cross\/$|^\/pas-seul\/$/i;
