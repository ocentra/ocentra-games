...
import { QUALITY_PARTIAL, QUALITY_VALUES } from "../../schema/types/quality.js";
...
import { EFFECT_TYPE_VALUES, COST_SOURCE_VALUES, LIMIT_TYPE_VALUES, COST_TYPE_FLAT, COST_TYPE_MATCH_CURRENT_BET, COST_TYPE_MIN_BET, COST_TYPE_MIN_RAISE, COST_TYPE_POT_LIMIT, COST_TYPE_BY_RANK, COST_VALUES, } from "@ocentra/game-domain/game/effect";
...
import { BANNED_DECK_VALUES } from "../../schema/data/banned-deck-values.js";
import { isValidDeckTriple } from "@ocentra/game-domain/deck/deckCompatibility";
import { CATEGORY_REQUIRED_MECHANICS, getDeckFamilySet, } from "@ocentra/game-domain/game/categoryMechanics";
import { validateOverridePaths } from "../../schema/data/override-paths.js";
import { isValidCategorySubcategoryPair, getCategorySubcategoryErrorMessage, } from "@ocentra/game-domain/game/categorySubcategoryMap";
import { getScoringRequirements, SCORING_FIELD_REQUIRED, } from "../../schema/data/scoring-requirements.js";
import { ZONE_TYPE_VALUES, ZONE_VISIBILITY_VALUES, ZONE_OWNER_VALUES, } from "@ocentra/game-domain/game/zone";
...
import { DRAW_SOURCE_VALUES, DRAW_VISIBILITY_VALUES, DRAW_TIMING_VALUES, } from "@ocentra/game-domain/deck/drawRules";
import { DISCARD_TIMING_VALUES, DISCARD_VISIBILITY_VALUES, } from "@ocentra/game-domain/game/discard";
import { INITIAL_DEAL_VALUES, HAND_DEFAULT_VALUES, STOCK_PURCHASE_VALUES, DISCARD_TOP_VALUES, TABLEAU_CARDS_VALUES, MARKET_CARDS_VALUES, } from "@ocentra/game-domain/game/cardVisibility";
import { LEGAL_STATUS_NA, LEGAL_STATUS_VALUES } from "../../schema/types/legal.js";
import { TAGS_VALUES } from "../../schema/types/tags.js";
...
import { DECLARATION_TYPE_VALUES, DECLARATION_ENCODING_VALUES, REVEAL_TIMING_VALUES, PIG_PENALTY_VALUES, } from "@ocentra/game-domain/game/declaration";
...
import { EXTRACTION_STATUS_VALUES, FIELD_STATUS_VALUES } from "../../schema/types/extraction.js";
import { BUY_CURRENCY_VALUES, NA_UNKNOWN_VALUES, NA_OR_NOT_APPLICABLE_VALUES } from "@ocentra/game-domain/game/buyCosts";
import { UI_THEMES_VALUES, MARKET_POSITION_VALUES, STOCK_POSITION_VALUES, DISCARD_POSITION_VALUES, PLAYER_HAND_LAYOUT_VALUES, POT_POSITION_VALUES, } from "../../schema/types/synthesis.js";
...
import { CAN_SUBSTITUTE_FOR_VALUES, ACTION_CARD_ACTION_VALUES, TARGET_PLAYER_VALUES, } from "@ocentra/game-domain/game/specialCards";
...
function allFreeOfPlaceholders(...values) {
    return values.every((v) => v == null || !containsPlaceholder(v));
}
function containsPagat(s) {
...
const costSchema = z.union([
    ...
    errorMap: () => ({
        message: "cost must be 0, NA, match_current_bet, min_bet, min_raise, pot_limit, byRank, or { type: 'flat', value: number }",
    }),
});
...
const playerActionSupportedTrue = z.object({
    ...
    description: z
        .string()
        .min(10)
        .refine((v) => !NA_UNKNOWN_VALUES.includes(v)),
    cost: costSchema,
    constraints: z
        .string()
        .min(5)
...
const playerActionSchema = z.union([playerActionSupportedFalse, playerActionSupportedTrue], {
    errorMap: () => ({
        message: "playerAction must be one of: (A) supported:true with real description/cost/constraints/effectType; " +
...
const phaseSchema = z
    ...
    cardVisibilityChanges: z.record(z.string().regex(/^[a-z][a-z0-9_]*$/), z.enum(VISIBILITY_VALUES)),
...
const overviewSchemaBase = z
    ...
    deck: z
        .string()
        .min(1)
        .refine((v) => !BANNED_DECK_VALUES.includes(v.trim()), {
    ...
    .strict();
const overviewSchema = overviewSchemaBase.refine((ov) => {
    const p = ov.players;
    return (ov.playerMode !== PLAYER_MODE_SINGLEPLAYER ||
...
const variationsSchema = z
    ...
    .strict()
    .refine((v) => v.list.length > 0 || (typeof v.noVariationsReason === "string" && v.noVariationsReason.trim().length >= 15), { message: "variations.list empty requires variations.noVariationsReason (min 15 chars), e.g. 'Sources describe only the standard form; no regional or rule variations documented.'", path: ["noVariationsReason"] });
const aiSchema = z
    .object({
    difficulty: z
        .object({
    ...
    if (!allNull || !noConsiderations)
        return true;
    const nr = a.nullReasons;
...
const playerActionsSchema = z
    .object(Object.fromEntries(ACTION_ID_VALUES.map((id) => [id, playerActionSchema])))
    .strict();
...
const playerConfigSchema = playerConfigSchemaBase
    .refine((pc) => pc.playerMode !== PLAYER_MODE_SINGLEPLAYER ||
    (pc.minPlayers === 1 && pc.maxPlayers === 1), { message: "singleplayer requires minPlayers=1 and maxPlayers=1" })
    .refine((pc) => pc.minPlayers <= pc.maxPlayers, { message: "engine.playerConfig.minPlayers must be <= maxPlayers" })
    .refine((pc) => !pc.partnerships ||
...
const engineSchemaBase = z
    ...
    deckDescription: z
        .string()
        .min(1)
        .refine((s) => wordCount(s) >= MIN_CUSTOM_DESCRIPTION_WORDS, { message: "deckDescription must be at least 15 words; provide a real explanation of the deck (e.g. composition, suits, ranks, any jokers)." })
        .optional(),
    suitDescription: z
        .string()
        .min(1)
        .refine((s) => wordCount(s) >= MIN_CUSTOM_DESCRIPTION_WORDS, { message: "suitDescription must be at least 15 words when present; provide a real explanation of the suit set." })
        .optional(),
    rankDescription: z
        .string()
        .min(1)
        .refine((s) => wordCount(s) >= MIN_CUSTOM_DESCRIPTION_WORDS, { message: "rankDescription must be at least 15 words when present; provide a real explanation of the rank set." })
        .optional(),
...
const engineSchema = engineSchemaBase
    .refine((e) => isValidDeckTriple(e.deckType, e.suitSet, e.rankSet), {
    message: "engine.deckType, suitSet, and rankSet must form a valid combination (see deck compatibility matrix)",
});
...
const _GameSchemaInner = z
    ...
    overview: reference(overviewSchema),
    history: reference(historySchema),
    setup: reference(setupSchema),
    rules: reference(rulesSchema),
    strategy: reference(strategySchema),
    variations: reference(variationsSchema),
    ai: reference(aiSchema),
    sources: reference(sourcesSchema),
    ...
    media: z
        ...
        .strict(),
    engine: reference(engineSchema),
    synthesis: reference(synthesisSchema),
    prompts: reference(promptsSchema),
    scoring: reference(scoringSchema),
    alsoKnownAs: z.union([z.array(z.string()), z.null()]),
    evidence: reference(evidenceSchema),
    extraction: reference(extractionSchema),
    fieldStatus: z.record(z.enum(FIELD_STATUS_VALUES)).optional(),
})
    .strict()
    .refine((g) => {
    const ov = g.overview;
    const pc = g.engine.playerConfig;
    return (ov.playerMode === pc.playerMode &&
        ov.players.minPlayers === pc.minPlayers &&
        ov.players.maxPlayers === pc.maxPlayers);
}, {
    message: "overview.playerMode, minPlayers, maxPlayers must match engine.playerConfig",
})
    .refine((g) => (g.overview.deckType ?? null) == null ||
    ...
    message: "synthesis.uiLayout.zones must only reference zone IDs that exist in engine.zones (UI layout must map to engine zones)",
    ...
    .refine((g) => {
    const p = g.overview.players;
    const rec = p.recommendedPlayers;
    if (rec == null || typeof rec !== "number")
        return true;
    return rec >= p.minPlayers && rec <= p.maxPlayers;
}, {
    message: "overview.players.recommendedPlayers must be between minPlayers and maxPlayers when present",
    path: ["overview", "players"],
})
    .refine((g) => validateOverridePaths(g.variations).valid, (g) => {
    const r = validateOverridePaths(g.variations);
    const paths = r.valid ? [] : r.invalidPaths;
    return {
    ...
    const actions = g.engine.playerActions;
    const customActions = (g.engine.customActions ?? []);
    for (const [_, pa] of Object.entries(actions)) {
        const hints = pa?.effectHints;
        if (!hints)
            continue;
    ...
    message: "effectHints (from, to, target) must reference zone IDs that exist in engine.zones",
    path: ["engine"],
})
    .refine((g) => {
    const validActions = new Set(Object.keys(g.engine.playerActions));
    ...
    for (const phase of g.engine.phases) {
        for (const aid of phase.legalActions) {
            if (!validActions.has(aid))
                return false;
        }
    }
    return true;
}, {
    message: "phase.legalActions must only reference action IDs that exist in engine.playerActions or engine.customActions",
    path: ["engine", "phases"],
})
    .refine((g) => {
    ...
    for (const phase of g.engine.phases) {
        ...
        for (const cond of phase.conditionalNext ?? []) {
            const np = cond.nextPhase;
            if (np != null && np.length > 0 && !phaseIds.has(np))
                return false;
        }
    }
    return true;
}, {
    message: "phase.nextPhase and conditionalNext[].nextPhase must reference phase IDs that exist in engine.phases",
    path: ["engine", "phases"],
})
    .refine((g) => {
    const validActions = new Set(Object.keys(g.engine.playerActions));
    ...
    for (const phase of g.engine.phases) {
        const cvc = phase.cardVisibilityChanges;
        if (!cvc)
            continue;
    ...
    message: "phase.cardVisibilityChanges keys must reference action IDs that exist in engine.playerActions or engine.customActions",
    path: ["engine", "phases"],
})
    .refine((g) => {
    const drawConfig = g.engine.drawConfig;
    if (drawConfig == null || !drawConfig.canDraw)
        return true;
    ...
    message: "drawConfig.drawSources reference zones (stock, market, discard, etc.) that must exist in engine.zones",
    ...
    if (cat === CATEGORY.POKER || cat === CATEGORY.VYING) {
        const hrs = eng.handRankingSystem;
    ...
    message: "Poker/Vying games require non-empty handRankingSystem; Trick-taking games require useTrump to be set",
    path: ["engine"],
})
    .superRefine((g, ctx) => {
    const cat = g.overview.category;
    if (cat == null || cat === CATEGORY.UNKNOWN)
        return;
    ...
    if (rules.configs != null) {
        for (const key of rules.configs) {
            const val = eng[key];
    ...
    if (new Set(ids).size !== ids.length)
        return false;
    for (const id of ids) {
        if (ACTION_ID_SET.has(id))
            return false;
    }
    return true;
}, {
    message: "engine.customActions must have unique ids that do not clash with standard action IDs",
    ...
    for (const [id, pa] of Object.entries(actions)) {
        if (pa?.supported === true)
            supported.add(id);
    }
    for (const ca of custom) {
        if (ca?.supported === true)
            supported.add(ca.id);
    }
    for (const phase of g.engine.phases) {
        for (const aid of phase.legalActions) {
            if (!supported.has(aid))
                return false;
        }
        const cvc = phase.cardVisibilityChanges;
        if (cvc) {
    ...
    message: "phase.legalActions and cardVisibilityChanges must only reference actions with supported: true",
    path: ["engine", "phases"],
})
    .refine((g) => {
    ...
    message: "engine.progression must only reference phase IDs that exist in engine.phases",
    ...
    if (g.legal.isCommercial) {
        ...
        if (note.length < 3) {
            ctx.addIssue({
                ...
                message: "commercial games must set legal.trademarkNote with the branded or trademarked name so risky titles stay quarantined from asset planning",
            });
        }
        if (g.legal.status === "Public Domain" || g.legal.status === "Patent-Expired") {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                path: ["legal", "status"],
                message: "commercial games cannot use legal.status 'Public Domain' or 'Patent-Expired'; use 'Trademarked', 'Proprietary', 'Unknown', or 'NA' with a reason",
            });
        }
    }
    if ((g.legal.status === "Trademarked" || g.legal.status === "Proprietary") && !g.legal.isCommercial) {
        ctx.addIssue({
            ...
            message: "legal.status 'Trademarked' or 'Proprietary' requires legal.isCommercial = true so commercial titles do not drive deck/card/ranking backlog",
        });
    }
    if (COMMERCIAL_DECK_TYPE_SET.has(g.engine.deckType) && !g.legal.isCommercial) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            path: ["legal", "isCommercial"],
            message: `engine.deckType '${g.engine.deckType}' is a commercial-only deck family in this repo, so the game must set legal.isCommercial = true`,
        });
    }
    ...
    if (!g.setup.hasPlaceholders) {
        const setupFields = ["players", "deck", "dealing"];
    ...
    nr(g.history.nullReasons, ["history"]);
    ...
    const on = g.overview.originName;
    const onNa = (v) => v == null || (typeof v === "string" && /^(NA|N\/A|Unknown|â€”|â€“|-|\.)$/i.test(v.trim()));
    if (onNa(on)) {
    ...
    if (!g.scoring.hasPlaceholders) {
        ...
        if (cvEmpty && req.cardValues === SCORING_FIELD_REQUIRED) {
            ...
            const r = g.scoring.nullReasons?.cardValues;
            if (typeof r !== "string" || r.trim().length < 15) {
                ctx.addIssue({
                    code: z.ZodIssueCode.custom,
                    path: ["scoring", "cardValues"],
    ...
    message: "overview.description, rules.objective, and rules.gameplay must not be empty or whitespace-only",
    path: ["overview"],
})
    .refine((g) => {
    ...
    message: "overview.description, rules.objective, rules.gameplay must not be lazy filler (Unknown, NA, TBD, TODO, placeholder)",
    path: ["overview"],
})
    .refine((g) => {
    if (g.overview.hasPlaceholders)
        return true;
    return allFreeOfPlaceholders(g.overview.description, g.overview.origin, g.overview.deck, g.overview.duration);
}, {
    message: "when overview.hasPlaceholders is false, overview content must not contain placeholder patterns (TBD, TODO, [PLACEHOLDER], etc.)",
    path: ["overview"],
})
    .refine((g) => {
    if (g.rules.hasPlaceholders)
    ...
    message: "when rules.hasPlaceholders is false, rules.objective and rules.gameplay must not contain placeholder patterns",
    path: ["rules"],
})
    .refine((g) => {
    if (g.setup.hasPlaceholders)
        return true;
    return allFreeOfPlaceholders(g.setup.players, g.setup.deck, g.setup.equipment, g.setup.dealing);
}, {
    message: "when setup.hasPlaceholders is false, setup content must not contain placeholder patterns",
    path: ["setup"],
})
    .refine((g) => {
    if (g.history.hasPlaceholders)
        return true;
    return allFreeOfPlaceholders(g.history.origins, g.history.evolution ?? "", g.history.cultural ?? "");
}, {
    message: "when history.hasPlaceholders is false, history content must not contain placeholder patterns",
    path: ["history"],
})
    .refine((g) => {
    if (g.strategy.hasPlaceholders)
        return true;
    return allFreeOfPlaceholders(g.strategy.basic ?? "", g.strategy.intermediate ?? "", g.strategy.advanced ?? "");
}, {
    message: "when strategy.hasPlaceholders is false, strategy content must not contain placeholder patterns",
    path: ["strategy"],
})
    .refine((g) => {
    if (g.quality !== "complete")
        return true;
    const sections = [
        g.overview.hasPlaceholders,
        g.history.hasPlaceholders,
    ...
    message: "quality 'complete' requires hasPlaceholders: false in all sections (overview, history, setup, rules, strategy, variations, ai, sources)",
    path: ["quality"],
})
    .refine((g) => {
    if (g.quality !== "complete")
    ...
    message: "quality 'complete' requires overview.category to be a real category, not null, Unknown, Other, or Miscellaneous",
    path: ["overview", "category"],
})
    .refine((g) => {
    if (g.quality !== "complete")
    ...
    message: "quality 'complete' with buyCosts enabled requires buyCosts.currency to be real (not Unknown)",
    ...
    message: "initialHandSize * maxPlayers must not exceed 500 (deck would be exhausted)",
    path: ["engine"],
})
    .refine((g) => {
    const dp = g.engine.dealPattern;
    if (dp == null || dp === "")
        return true;
    ...
    .refine((g) => {
    const hrs = g.engine.handRankingSystem;
    if (hrs == null || hrs === "")
        return true;
    ...
    message: "marketConfig.enabled: true requires a market zone in engine.zones",
    ...
    if (bc == null || !bc.enabled)
        return true;
    const zoneIds = new Set(g.engine.zones.map((z) => z.id));
    const sources = bc.sources;
    if (sources.market != null && !zoneIds.has("market"))
        return false;
    if (sources.stock != null && !zoneIds.has("stock"))
        return false;
    return true;
}, {
    message: "buyCosts.enabled: true requires market and stock zones when those sources are configured",
    path: ["engine", "buyCosts"],
})
    .refine((g) => {
    const tc = g.engine.trickConfig;
    if (tc == null || !tc.hasTricks)
        return true;
    const zoneIds = new Set(g.engine.zones.map((z) => z.id));
    return zoneIds.has("trick");
}, {
    message: "trickConfig.hasTricks: true requires a trick zone in engine.zones",
    path: ["engine", "trickConfig"],
})
    .refine((g) => {
    const p = g.overview.players;
    ...
    .refine((g) => {
    const tc = g.engine.trickConfig;
    if (tc?.bidding == null || !tc.bidding.hasBidding)
        return true;
    const b = tc.bidding;
    const minB = b.minBid;
    const maxB = b.maxBid;
    ...
    message: "phase.actor role:xxx requires that role to exist in engine.roles",
    path: ["engine", "phases"],
})
    .refine((g) => {
    ...
    .refine((g) => g.variations.list.every((v) => {
    ...
    message: "Each variation with empty overrides must have emptyOverridesReason (min 15 chars), e.g. 'Find more info and add overrides so we can make game.' Either add engine overrides for the variant or explain what is needed.",
    path: ["variations", "list"],
})
    .refine((g) => {
    const eng = g.engine;
    const bl = eng.bettingLimits;
    const nonBetting = ["None", "NA", "Unknown"];
    if (bl == null || nonBetting.includes(bl))
        return true;
    const ss = eng.playerConfig?.startingStack;
    if (ss == null)
        return true;
    return ss > 0;
}, {
    message: "betting game (bettingLimits not none) requires startingStack > 0 when set",
    path: ["engine", "playerConfig"],
});
function requireNAR(engine, key) {
    const r = engine.notApplicableReasons?.[key];
    return typeof r === "string" && r.trim().length >= 15;
}
function requireNullReason(section, key) {
    const r = section.nullReasons?.[key];
    return typeof r === "string" && r.trim().length >= 15;
}
const NA_STRINGS = new Set(["NA", "not applicable", "Unknown", null]);
function isConfigFilled(val) {
    if (val === null || val === undefined)
        return false;
    if (typeof val === "string" && NA_STRINGS.has(val))
        return false;
    return typeof val === "object";
}
export const GameSchema = _GameSchemaInner
    .refine((g) => !g.completeness.overview || !g.overview.hasPlaceholders, { message: "completeness.overview cannot be true when overview.hasPlaceholders is true", path: ["completeness", "overview"] })
    .refine((g) => !g.completeness.history || !g.history.hasPlaceholders, { message: "completeness.history cannot be true when history.hasPlaceholders is true", path: ["completeness", "history"] })
    .refine((g) => !g.completeness.setup || !g.setup.hasPlaceholders, { message: "completeness.setup cannot be true when setup.hasPlaceholders is true", path: ["completeness", "setup"] })
    .refine((g) => !g.completeness.rules || !g.rules.hasPlaceholders, { message: "completeness.rules cannot be true when rules.hasPlaceholders is true", path: ["completeness", "rules"] })
    .refine((g) => !g.completeness.strategy || !g.strategy.hasPlaceholders, { message: "completeness.strategy cannot be true when strategy.hasPlaceholders is true", path: ["completeness", "strategy"] })
    .refine((g) => !g.completeness.variations || !g.variations.hasPlaceholders, { message: "completeness.variations cannot be true when variations.hasPlaceholders is true", path: ["completeness", "variations"] })
    .refine((g) => !g.completeness.ai || !g.ai.hasPlaceholders, { message: "completeness.ai cannot be true when ai.hasPlaceholders is true", path: ["completeness", "ai"] })
    .refine((g) => !g.completeness.sources || !g.sources.hasPlaceholders, { message: "completeness.sources cannot be true when sources.hasPlaceholders is true", path: ["completeness", "sources"] })
    .superRefine((g, ctx) => {
    ...
    message: "overview.category is null or Unknown. Either set a real category from the primary source (e.g. Poker, Trick-taking), or add extraction.overviewNaReasons.category (min 15 characters) explaining why it is not applicable for this game.",
    path: ["overview", "category"],
})
    .refine((g) => {
    const cat = g.overview.category;
    ...
    message: "overview.category cannot be Other or Miscellaneous. Determine the correct category from the primary source (e.g. Poker, Trick-taking, Shedding, Rummy, Domino, War, Banking). If no existing category fits, extend the category list â€” Other and Miscellaneous are not allowed.",
    path: ["overview", "category"],
})
    .refine((g) => {
    const origin = (g.overview.origin ?? "").trim();
    const naLike = /^(NA|N\/A|Unknown|â€”|â€“|-|\.)$/i;
    if (origin.length > 0 && !naLike.test(origin))
        return true;
    const r = g.extraction.overviewNaReasons?.origin;
    return typeof r === "string" && r.trim().length >= 15;
}, {
    message: "overview.origin is empty or NA/Unknown. Either set a real origin (e.g. country/region) from the primary source, or add extraction.overviewNaReasons.origin (min 15 characters) explaining why it is not applicable for this game.",
    path: ["overview", "origin"],
})
    .refine((g) => {
    const duration = (g.overview.duration ?? "").trim();
    const naLike = /^(NA|N\/A|Unknown|â€”|â€“|-|\.)$/i;
    if (duration.length > 0 && !naLike.test(duration))
        return true;
    const r = g.extraction.overviewNaReasons?.duration;
    return typeof r === "string" && r.trim().length >= 15;
}, {
    message: "overview.duration is empty or NA/Unknown. Either set a real duration (e.g. '15-30 minutes') from the primary source, or add extraction.overviewNaReasons.duration (min 15 characters) explaining why it is not applicable for this game.",
    path: ["overview", "duration"],
})
    .refine((g) => {
    const sc = g.overview.subCategory;
    if (sc != null && typeof sc === "string" && sc.trim().length > 0)
        return true;
    const r = g.extraction.overviewNaReasons?.subCategory;
    return typeof r === "string" && r.trim().length >= 15;
}, {
    message: "overview.subCategory is required; cannot be null, NA, or placeholder. Set a real subCategory from the primary source (e.g. Scopa Family for Fishing, Community Card for Poker).",
    ...
    if (cat == null || ["Unknown", "Other", "Miscellaneous"].includes(cat) || naLike.test(String(cat ?? "").trim()))
        return false;
    if (sc == null || typeof sc !== "string" || sc.trim().length === 0 || naLike.test(sc.trim()))
        return false;
    ...
    if (cat == null ||
        ...
        msg = getCategorySubcategoryErrorMessage(cat ?? "", sc ?? "");
    return { message: msg, path: ["overview", "subCategory"] };
})
    .refine((g) => {
    if (g.extraction.status !== "validated")
        return true;
    const mc = g.extraction.missingCritical;
    return mc == null || mc.length === 0;
}, {
    message: "extraction.missingCritical must be empty when status is 'validated'. Fill every critical field from the primary source (sources.primary URL or HTML); do not mark validated until you have read the source and filled missing data.",
    ...
    message: "strategy must have real content (at least one of basic, intermediate, advanced, or tips) when status is 'validated'. Read the primary source (sources.primary URL or HTML) for this game and add strategy content from the source.",
    path: ["strategy"],
})
    .refine((g) => {
    if (g.extraction.status !== "validated")
        return true;
    if (!g.variations.hasPlaceholders || g.variations.list.length > 0)
        return true;
    return false;
}, {
    message: "variations.list cannot be empty when status is 'validated' and variations.hasPlaceholders is true. Read the primary source (sources.primary URL or HTML) for this game and add at least one variation with real name and description from the source.",
    path: ["variations"],
})
    .refine((g) => {
    if (g.extraction.status !== "validated")
        return true;
    return (!containsStubPattern(g.overview.description) &&
    ...
    message: "quality cannot be 'stub' when status is 'validated'. Read the primary source (sources.primary URL or HTML) and fill overview, history, setup, rules with real content so quality can be at least partial or draft; then set quality accordingly.",
    path: ["quality"],
})
    .refine((g) => {
    if (g.extraction.status !== "validated")
    ...
    return (c.overview === true || c.history === true || c.setup === true || c.rules === true);
}, {
    message: "At least one of completeness.overview, .history, .setup, .rules must be true when status is 'validated'. Read the primary source (sources.primary URL or HTML) and fill the corresponding sections with real content so at least one completeness flag is true.",
    path: ["completeness"],
})
    .refine((g) => {
    if (g.extraction.status !== "validated")
    ...
    message: "rules.keyRules cannot be only 'Follow the game rules for your variant' when status is 'validated'. Read the primary source (sources.primary URL or HTML) and list the actual key rules (e.g. matching rules, win condition, special cards).",
    path: ["rules", "keyRules"],
})
    .refine((g) => {
    if (g.extraction.status !== "validated")
    ...
    message: "overview.origin and overview.duration must be non-empty and not NA/Unknown when status is 'validated'. Read the primary source (sources.primary URL or HTML) and set origin (e.g. country/region) and duration (e.g. '15-30 minutes') from the source.",
    path: ["overview"],
})
    .refine((g) => {
    if (g.extraction.status !== "validated")
    ...
    message: "overview.description must be at least 80 characters when status is 'validated'. Read the primary source (sources.primary URL or HTML) and write a real summary (what the game is, how it works); do not use a one-liner or 'see X for full rules'.",
    path: ["overview", "description"],
})
    .refine((g) => {
    if (g.scoring.hasPlaceholders)
        return true;
    return allFreeOfPlaceholders(g.scoring.description, g.scoring.winCondition, typeof g.scoring.splitRules === "string" ? g.scoring.splitRules : null);
}, {
    ...
    .refine((g) => {
    if (g.sources.hasPlaceholders)
        return true;
    const naSet = new Set(NA_UNKNOWN_VALUES);
    return g.sources.primary.some((s) => !naSet.has(s.id));
}, {
    message: "sources.primary must contain at least one source with a real id (not NA/Unknown) when sources.hasPlaceholders is false. Read the actual source (URL or HTML) for this game and set the source id and name from it; do not leave placeholder ids.",
    ...
    message: "sources.primary source names must not contain placeholder text when sources.hasPlaceholders is false. Read the actual source (URL or HTML) and use the real title or site name (e.g. 'Pagat.com - Game Name'); do not leave placeholder names.",
    path: ["sources", "primary"],
})
    .refine((g) => {
    ...
    .refine((g) => g.evidence.every((e) => !containsPlaceholder(e.path)), {
    ...
    if (g.variations.hasPlaceholders)
        return true;
    return g.variations.list.every((v) => !containsPlaceholder(v.name) && !containsPlaceholder(v.description));
}, {
    ...
    message: "engine.drawConfig: null requires engine.notApplicableReasons.drawConfig â€” " +
    ...
    message: "engine.discardConfig: null requires engine.notApplicableReasons.discardConfig â€” " +
    ...
    if (sc !== null && !NA_STRINGS.has(sc))
        return true;
    return requireNAR(g.engine, "specialCards");
}, {
    message: "engine.specialCards: null/NA requires engine.notApplicableReasons.specialCards â€” " +
    ...
    if (sh !== null && !NA_STRINGS.has(sh))
        return true;
    return requireNAR(g.engine, "shedding");
}, {
    message: "engine.shedding: null/NA requires engine.notApplicableReasons.shedding â€” " +
    ...
    if (fc !== null && !NA_STRINGS.has(fc))
        return true;
    return requireNAR(g.engine, "fishingConfig");
}, {
    message: "engine.fishingConfig: null/NA requires engine.notApplicableReasons.fishingConfig â€” " +
    ...
    if (rc !== null && !NA_STRINGS.has(rc))
        return true;
    return requireNAR(g.engine, "roundConfig");
}, {
    message: "engine.roundConfig: null/NA requires engine.notApplicableReasons.roundConfig â€” " +
    ...
    .refine((g) => {
    if (g.history.hasPlaceholders || g.history.evolution !== null)
        return true;
    return requireNullReason(g.history, "evolution");
}, {
    message: "history.evolution: null requires history.nullReasons.evolution when hasPlaceholders=false â€” " +
        "e.g. 'No documented evolutionary changes; game emerged as a stable folk tradition.'",
    path: ["history", "evolution"],
})
    .refine((g) => {
    if (g.history.hasPlaceholders || g.history.cultural !== null)
        return true;
    return requireNullReason(g.history, "cultural");
}, {
    message: "history.cultural: null requires history.nullReasons.cultural when hasPlaceholders=false â€” " +
        "e.g. 'No specific cultural or regional significance documented beyond its folk origins.'",
    path: ["history", "cultural"],
})
    .refine((g) => {
    if (g.strategy.hasPlaceholders || g.strategy.basic !== null)
        return true;
    return requireNullReason(g.strategy, "basic");
}, {
    message: "strategy.basic: null requires strategy.nullReasons.basic when hasPlaceholders=false â€” " +
        "e.g. 'Animals is a pure memory game; strategy consists only of remembering card positions.'",
    path: ["strategy", "basic"],
})
    .refine((g) => {
    if (g.strategy.hasPlaceholders || g.strategy.intermediate !== null)
        return true;
    return requireNullReason(g.strategy, "intermediate");
}, {
    message: "strategy.intermediate: null requires strategy.nullReasons.intermediate when hasPlaceholders=false â€” " +
        "e.g. 'No distinct intermediate level exists; the game scales only with memory capacity.'",
    path: ["strategy", "intermediate"],
})
    .refine((g) => {
    if (g.strategy.hasPlaceholders || g.strategy.advanced !== null)
        return true;
    return requireNullReason(g.strategy, "advanced");
}, {
    message: "strategy.advanced: null requires strategy.nullReasons.advanced when hasPlaceholders=false â€” " +
    ...
    .refine((g) => {
    if (g.ai.hasPlaceholders || g.ai.difficulty.easy !== null)
        return true;
    return requireNullReason(g.ai, "easy");
}, {
    message: "ai.difficulty.easy: null requires ai.nullReasons.easy when hasPlaceholders=false â€” " +
        "e.g. 'Easy AI flips cards at random with no memory tracking.'",
    path: ["ai", "difficulty", "easy"],
})
    .refine((g) => {
    if (g.ai.hasPlaceholders || g.ai.difficulty.medium !== null)
        return true;
    return requireNullReason(g.ai, "medium");
}, {
    message: "ai.difficulty.medium: null requires ai.nullReasons.medium when hasPlaceholders=false â€” " +
        "e.g. 'No distinct medium tier; all non-easy AI plays at full memory recall.'",
    path: ["ai", "difficulty", "medium"],
})
    .refine((g) => {
    if (g.ai.hasPlaceholders || g.ai.difficulty.hard !== null)
        return true;
    return requireNullReason(g.ai, "hard");
}, {
    message: "ai.difficulty.hard: null requires ai.nullReasons.hard when hasPlaceholders=false â€” " +
    ...
    message: "scoring.targetScore: null/NA requires scoring.nullReasons.targetScore when hasPlaceholders=false â€” " +
        "e.g. 'No fixed target score; play ends when all pairs are collected and winner has the most pairs.'",
    path: ["scoring", "targetScore"],
})
    .refine((g) => {
    if (g.scoring.hasPlaceholders || g.scoring.scoringDirection !== null)
        return true;
    return requireNullReason(g.scoring, "scoringDirection");
}, {
    message: "scoring.scoringDirection: null requires scoring.nullReasons.scoringDirection when hasPlaceholders=false â€” " +
