...
function extractUrl(filePath: string): string | null {
  ...
  if (m) return "https://www.pagat.com" + m[1].replace(/^\//, "/").trim();
  ...
  if (fileName.startsWith("parlett-")) {
    ...
    if (!base) {
      if (head.includes("histocg.css")) base = "https://www.parlettgames.uk/histocg";
      else if (slug.startsWith("minimis")) base = "https://www.parlettgames.uk/oricards";
      else base = "https://www.parlettgames.uk/histocs";
    }
  ...
  if (fileName.startsWith("riichi-wiki")) return "https://riichi.wiki/";
  if (fileName.startsWith("scribd-")) return "https://www.scribd.com/document/House-of-Cards-Game";
  if (fileName.startsWith("norskespilleautomater-"))
    return "https://norskespilleautomater.io/kortspill/kanin";
  if (fileName.startsWith("ispa-")) return "https://www.ispa-world.org/";
  if (fileName.startsWith("namu-")) return "https://namu.wiki/w/%EA%B3%A0%EC%8A%A4%ED%86%B1";
  if (fileName.startsWith("piatnik-")) return "https://www.piatnik.com/en/games/cards/rainbow";
  if (fileName.startsWith("rainbowthecardgame")) return "https://rainbowthecardgame.com/";
...
const files = readdirSync(SOURCE_HTML).filter((f: string) => f.endsWith(".html"));

for (const f of files) {
  const fp = join(SOURCE_HTML, f);
  const url = extractUrl(fp);
  if (url) manifest[f] = url;
  if (/^wiki-/.test(f)) {
...
if (Object.keys(redirects).length) output._redirects = redirects;
