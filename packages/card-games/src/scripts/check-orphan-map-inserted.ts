...
function isInserted(normUrl: string): boolean {
...
const map = JSON.parse(readFileSync(MAP_FILE, "utf8")) as Record<string, string>;
interface InsertedInfo {
  url: string;
  jsonFile: string;
}
const inserted: InsertedInfo[] = [];
const notInserted: InsertedInfo[] = [];
