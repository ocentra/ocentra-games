...
function isGenericUrl(url: string): boolean {
  try {
    ...
    if (/pagat\.com$/i.test(u.hostname) && (path === "/" || path === "" || /^\/index\.html?$/i.test(path)))
      return true;
    if (/wikipedia\.org/i.test(u.hostname) && /\/wiki\/List_of/i.test(pathLower)) return true;
    if (/boardgamegeek\.com$/i.test(u.hostname) && (path === "/" || path === "")) return true;
...
if (existsSync(MANIFEST_PATH)) {
  ...
  for (const [file, url] of Object.entries(manifest)) {
    if (file === "_redirects" || !url) continue;
  ...
  const redirects = (manifestData._redirects as Record<string, string>) || {};
...
const h1Re = /<h1[^>]*>([^<]+)<\/h1>/i;
const manifestTitleToFile = new Map<string, string>();
for (const f of existsSync(SOURCE_HTML) ? readdirSync(SOURCE_HTML).filter((x: string) => x.endsWith(".html")) : []) {
  try {
    ...
    if (title) {
      const n = title
        ...
        .replace(/\s+/g, " ")
        .trim();
      if (n && !manifestTitleToFile.has(n)) manifestTitleToFile.set(n, f);
    }
  } catch {
    void 0;
  }
...
for (const [jsonFile, entry] of byJson) {
  for (const u of getUrlsFromJson(jsonFile)) entry.urls.add(u);
}
...
interface SectionItem {
  ...
  primary: string;
  firstUrl: string;
  localHtmlFiles: Set<string>;
  urlList: string[];
}

const bySection = new Map<string, SectionItem[]>();
for (const c of "ABCDEFGHIJKLMNOPQRSTUVWXYZ0") bySection.set(c, []);
...
for (const [jsonFile, { names, urls }] of byJson) {
  ...
  const localHtmlFiles = new Set<string>();
  for (const u of urlList) {
    if (isGenericUrl(u)) continue;
    const n = normalizeUrl(u);
    if (urlToHtmlFiles.has(n)) {
      for (const f of urlToHtmlFiles.get(n)!) {
        if (sourceHtmlFiles.has(f)) localHtmlFiles.add(f);
      }
    }
    const slugFile = slug(u) + ".html";
    if (sourceHtmlFiles.has(slugFile)) localHtmlFiles.add(slugFile);
  }
  ...
  if (localHtmlFiles.size === 0 && hasUsefulUrl && allNames.length > 0) {
    ...
    for (const n of allNames) {
      const w = norm(n)
        ...
        .slice(0, 3)
        .join(" ");
      if (w) gameWords.add(w);
    }
    for (const [manifestTitle, file] of manifestTitleToFile) {
      if (
        sourceHtmlFiles.has(file) &&
        [...gameWords].some(
          (w) =>
            manifestTitle.includes(w) || w.split(" ").every((x) => manifestTitle.includes(x))
        )
      ) {
