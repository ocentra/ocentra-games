...
function* walkJsonFilesLazy(dir: string): Generator<string> {
  const stack = [dir];
  while (stack.length) {
    const d = stack.pop()!;
    const entries = fs.readdirSync(d, { withFileTypes: true });
...
function* expandTargetsLazy(targets: string[], prefix: string | null = null): Generator<string> {
  ...
  for (const t of targets) {
    ...
    if (isDir(abs)) {
      ...
      } else die(`FAIL: Target not found: ${t}`);
    }
  }
}
...
function isDeckRelatedError(path: string, message: string): boolean {
  if (DECK_ERROR_PATHS.some((p) => path === p || path.startsWith(p + "."))) return true;
  if (/Custom is disallowed/i.test(message)) return true;
  if (/deckDescription must be/i.test(message)) return true;
  if (/suitDescription must be/i.test(message)) return true;
  if (/rankDescription must be/i.test(message)) return true;
  if (/String must contain at least 1 character/.test(message) && (path.includes("deckDescription") || path.includes("suitDescription") || path.includes("rankDescription"))) return true;
  return false;
}
...
interface ParseArgsResult {
  ...
  listFailuresLimit: number | null;
  prefix: string | null;
  skipUrlCheck: boolean;
  schemaOnly: boolean;
  deckErrorsOnly: boolean;
  strict: boolean;
}

function parseArgs(argv: string[]): ParseArgsResult {
  const args: ParseArgsResult = {
    targets: [],
    failFast: false,
    quiet: false,
    json: false,
  ...
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === "--fail-fast") args.failFast = true;
      ...
      if (eq !== -1) {
        ...
        if (!Number.isInteger(n) || n < 1) die(`Invalid --list-failures limit: ${a.slice(eq + 1)}`);
      ...
      if (!Number.isInteger(n) || n < 1) die(`Invalid --list-failures limit: ${a.slice("--list-failures=".length)}`);
      args.listFailuresLimit = n;
    } else if (/^-l\d+$/.test(a)) {
      args.listFailures = true;
      ...
      if (args.listFailuresLimit < 1) die(`Invalid limit: ${a}`);
    } else if (a === "--prefix" || a === "-p") {
      if (argv[i + 1] == null) die("Missing value for --prefix");
    ...
    } else if (a === "--skip-url-check" || a === "--no-verify-urls") args.skipUrlCheck = true;
    else if (a === "--schema-only") args.schemaOnly = true;
    else if (a === "--deck-errors-only" || a === "--list-deck-errors") args.deckErrorsOnly = true;
    else if (a === "--strict" || a === "-s") args.strict = true;
    else if (a === "--help" || a === "-h") {
      console.log(`
Usage:
...
function expandTargets(targets: string[]): string[] {
  ...
  for (const t of targets) {
    const abs = path.isAbsolute(t) ? t : path.join(process.cwd(), t);
    if (isDir(abs)) files.push(...walkJsonFiles(abs));
    else if (isFile(abs)) files.push(abs);
    else {
      const alt = path.join(path.dirname(PROCESSED_GAMES_DIR), t);
      if (isDir(alt)) files.push(...walkJsonFiles(alt));
      else if (isFile(alt)) files.push(alt);
      else die(`FAIL: Target not found: ${t}`);
    }
  }
...
async function checkPrimaryUrlsReachable(data: Record<string, unknown>): Promise<{ ok: boolean; msg: string }> {
  ...
  for (const url of urls) {
    if (await isUrlReachable(url)) return { ok: true, msg: "" };
  }
  return {
    ok: false,
...
function ruleLabel(path: string, message: string): string {
  ...
  if (path === "overview.category") return "overview.category";
  if (path === "overview.difficulty") return "overview.difficulty";
  if (path === "overview.description") return "overview.description";
  if (path === "overview") return "overview (origin/duration)";
  if (path === "extraction.status") {
    if (/missingCritical|missing critical/i.test(message)) return "extraction.status";
    if (/Pagat|external sites/i.test(message)) return "Key fields (Pagat/stub)";
    return "extraction.status";
  }
  if (path === "strategy") return "strategy";
  if (path === "variations") return "variations";
  if (path === "quality") return "quality";
  if (path === "completeness") return "completeness";
  if (path === "rules.keyRules") return "rules.keyRules";
  if (path.startsWith("variations.")) return path;
...
function issueSummary(message: string, maxLen = 120): string {
...
function printFailureSummary(basename: string, errors: ValidationError[]): void {
  ...
  if (onlyOverviewNa) {
    console.error(
      "Overview has NA, Unknown, or empty in category, difficulty, origin, or duration. Either set real values from the primary source or add extraction.overviewNaReasons with a reason (min 15 characters) for each.\n"
    );
  } else {
    console.error(
      "The validator found issues (e.g. stub/placeholder data, or NA/Unknown without a reason). Fix the fields below or add extraction.overviewNaReasons where NA is valid for this game.\n"
    );
  }
...
function validateFile(filePath: string, data: unknown): { ok: true; game: Game } | { ok: false; errors: ValidationError[] } {
...
(async function main(): Promise<void> {
  ...
  const fileIterator: Iterable<string> = useLazyLimit
    ? expandTargetsLazy(args.targets, args.prefix)
    : files;
  if (!useLazyLimit && files.length === 0) die("FAIL: No JSON files found.");

  let okCount = 0;
  let failCount = 0;
  const failures: Array<{ file: string; reasons: string[] }> = [];
  const results: Array<{ file: string; valid: boolean; errors: ValidationError[] }> = [];
  ...
  for (const file of fileIterator) {
    ...
    if (!validation.ok) {
      ...
      if (useLazyLimit && failures.length >= args.listFailuresLimit!) break;
      continue;
    }
    ...
    if (!args.skipUrlCheck) {
      ...
      if (!urlResult.ok) {
        failCount++;
        if (!args.deckErrorsOnly) failures.push({ file: basename, reasons: [urlResult.msg] });
        if (args.json) {
          results.push({
            file: basename,
            valid: false,
  ...
  if (useLazyLimit && fileCount === 0) die("FAIL: No JSON files found.");
  ...
  if (args.listFailures) {
    ...
    console.log(
      JSON.stringify({ ok: okCount, fail: failCount, results }, null, 2)
    );
  } else if (!args.quiet) {
