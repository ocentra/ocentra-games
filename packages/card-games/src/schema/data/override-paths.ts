...
export function isValidOverridePath(path: string): boolean {
  if (UNSAFE_PATH_PATTERNS.test(path)) return false;
  if (VALID_PATHS.has(path)) return true;
  const m = path.match(/^engine\.constants\.([a-z_]+)$/);
  if (m == null) return false;
  return (CONSTANTS_KEYS as readonly string[]).includes(m[1]);
}

const NUMERIC_CONSTANT_PATTERN = /^engine\.constants\./;

function isValidBuySourceMarket(value: unknown): boolean {
  if (value == null || typeof value !== "object") return false;
  const o = value as Record<string, unknown>;
  if (typeof o.flat !== "number" && o.flat !== null) return false;
  if (o.byRank != null && (typeof o.byRank !== "object" || Array.isArray(o.byRank))) return false;
  return true;
}

function isValidBuySourceStock(value: unknown): boolean {
  if (value == null || typeof value !== "object") return false;
  const o = value as Record<string, unknown>;
  return typeof o.flat === "number" && Number.isFinite(o.flat);
}

function isValidBuySourceDiscard(value: unknown): boolean {
  if (value === null || NA_UNKNOWN_VALUES.includes(value as (typeof NA_UNKNOWN_VALUES)[number]))
    return true;
...
function isValidOverrideValue(path: string, value: unknown): boolean {
  ...
  switch (path) {
    ...
    case "engine.cardVisibility.stockPurchase":
      return typeof value === "string" && (STOCK_PURCHASE_VALUES as readonly string[]).includes(value);
    case "engine.cardVisibility.handDefault":
      return typeof value === "string" && (HAND_DEFAULT_VALUES as readonly string[]).includes(value);
    case "engine.cardVisibility.discardTop":
      return typeof value === "string" && (DISCARD_TOP_VALUES as readonly string[]).includes(value);
...
export function validateOverridePaths(
  variations: { list?: Array<{ overrides?: Record<string, unknown> }> }
): { valid: true } | { valid: false; invalidPaths: string[] } {
  const invalid: string[] = [];
  for (const v of variations.list ?? []) {
    for (const [key, val] of Object.entries(v.overrides ?? {})) {
      if (!isValidOverridePath(key)) invalid.push(key);
      else if (!isValidOverrideValue(key, val)) invalid.push(key);
    }
  }
