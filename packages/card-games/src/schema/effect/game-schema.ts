...
function reference<T>(schema: schema.SchemaType<T>): schema.SchemaType<T> {
  return schema;
}
...
const nullReasonsSchema = schema
  .record(
    schema.string(),
    schema
      .string()
...
const actionIdEnum = schema.enum(ACTION_ID_VALUES);
const actionRefSchema = schema.union([
  actionIdEnum,
  schema.string().regex(/^[a-z][a-z0-9_]*$/),
]);

const costSchema = schema.union(
  [
    schema.object({ type: schema.literal(COST_TYPE_FLAT), value: schema.number().int().min(0) }),
    schema.object({ type: schema.literal(COST_TYPE_MATCH_CURRENT_BET) }),
    schema.object({ type: schema.literal(COST_TYPE_MIN_BET) }),
    schema.object({ type: schema.literal(COST_TYPE_MIN_RAISE) }),
    schema.object({ type: schema.literal(COST_TYPE_POT_LIMIT) }),
    schema.object({ type: schema.literal(COST_TYPE_BY_RANK) }),
    schema.literal(COST_VALUES[0]),
    schema.literal(COST_VALUES[1]),
  ],
  {
    errorMap: () => ({
      message:
...
const zoneIdSchema = schema
  .string()
  .refine(isValidZoneId, {
    message: "Zone ID must be an engine zone (e.g. hand, stock, market, pot) or custom:name format",
  });

const effectHintsDrawSchema = schema
  .object({
    from: zoneIdSchema.optional(),
    to: zoneIdSchema.optional(),
    count: schema.number().int().min(1).optional(),
  })
  .strict();

const effectHintsBetSchema = schema
  .object({
    amount: schema.string().optional(),
    min: schema.number().int().min(0).optional(),
    max: schema.number().int().min(0).optional(),
    limitType: schema.enum(LIMIT_TYPE_VALUES).optional(),
  })
  .strict();

const effectHintsBuySchema = schema
  .object({
    from: zoneIdSchema.optional(),
    to: zoneIdSchema.optional(),
    costSource: schema.enum(COST_SOURCE_VALUES).optional(),
  })
  .strict();

const effectHintsPlaySchema = schema
  .object({
    from: zoneIdSchema.optional(),
    to: zoneIdSchema.optional(),
  })
...
const effectHintsDiscardSchema = schema
  .object({
    from: zoneIdSchema.optional(),
    to: zoneIdSchema.optional(),
    count: schema.number().int().min(0).optional(),
  })
  .strict();

const effectHintsDealSchema = schema
  .object({
...
const effectHintsPassSchema = schema.object({}).strict();
const effectHintsFoldSchema = schema.object({}).strict();

const effectHintsAnteSchema = schema
  .object({ amount: schema.string().optional() })
  .strict();

const effectHintsBidSchema = schema
  .object({
    min: schema.number().int().min(0).optional(),
    max: schema.number().int().min(0).optional(),
    declarationType: schema.string().optional(),
  })
  .strict();

const effectHintsSchema = schema.union(
  [
...
const playerActionSupportedFalse = schema.object({
  supported: schema.literal(false),
  description: schema.enum(NA_OR_NOT_APPLICABLE_VALUES),
  cost: schema.literal(NA_UNKNOWN_VALUES[0]),
  constraints: schema.enum(NA_OR_NOT_APPLICABLE_VALUES),
  isTerminating: schema.boolean(),
  effectType: schema.enum(NA_OR_NOT_APPLICABLE_VALUES),
  effectHints: schema.record(schema.unknown()).optional(),
  id: schema.string().regex(/^[a-z][a-z0-9_]*$/).optional(),
  reason: schema
    .string()
...
const playerActionSupportedTrue = schema.object({
...
const phaseActorSchema = schema.union([
  schema.enum(PHASE_ACTOR_VALUES),
  schema.string().regex(/^role:[a-z][a-z0-9_]*$/),
  schema.string().regex(/^custom:[a-z][a-z0-9_]*$/),
]);

const phaseSchema = schema
  .object({
    id: schema.string().regex(/^[a-z][a-z0-9_]*$/),
    idTemplate: schema.string().optional(),
    repeatCount: schema.number().int().min(1).optional(),
    label: schema.string(),
    actor: phaseActorSchema,
    legalActions: schema.array(actionRefSchema),
    nextPhase: schema.union([
      schema.null(),
      schema.string().regex(/^[a-z][a-z0-9_]*$/),
    ]),
    isMandatory: schema.boolean(),
    loopIndex: schema.union([schema.number().int(), schema.null()]),
    totalLoops: schema.union([schema.number().int(), schema.null()]),
    conditionalNext: schema.union([
      schema.null(),
      schema.array(
        schema
          .object({
    ...
    cardVisibilityChanges: schema.record(
      schema.string().regex(/^[a-z][a-z0-9_]*$/),
      schema.enum(VISIBILITY_VALUES)
    ),
    notes: schema.union([schema.string(), schema.null()]),
  })
  .strict();

const completenessSchema = schema
  .object({
    overview: schema.boolean(),
    history: schema.boolean(),
    setup: schema.boolean(),
    rules: schema.boolean(),
    strategy: schema.boolean(),
    variations: schema.boolean(),
    ai: schema.boolean(),
    sources: schema.boolean(),
  })
  .strict();

const playersOverviewSchema = schema
  .object({
    minPlayers: schema.number().int().min(1),
    maxPlayers: schema.number().int().min(1),
    recommendedPlayers: schema.union([schema.number().int().min(1), schema.null()]).optional(),
    display: schema.union([schema.string(), schema.null()]).optional(),
  })
  .strict();

const overviewSchemaBase = schema
  .object({
    description: schema.string(),
    category: schema.enum(CATEGORY_VALUES),
    subCategory: schema.union([schema.enum(SUB_CATEGORY_VALUES), schema.null()]),
    origin: schema.string(),
    originName: schema.union([schema.string().min(1), schema.null()]).optional(),
    playerMode: schema.enum(PLAYER_MODE_VALUES),
    players: playersOverviewSchema,
    deckType: schema.enum(DECK_TYPE_VALUES).optional(),
    deck: schema
      .string()
      .min(1)
    ...
    difficulty: schema.enum(DIFFICULTY_VALUES),
    duration: schema.string(),
    hasPlaceholders: schema.boolean(),
  })
  .strict();
...
const historySchema = schema
  .object({
    origins: schema.string().min(15),
    originCountries: schema
      .array(schema.string().min(1).max(80).regex(ORIGIN_COUNTRY_PATTERN, {
    ...
    timeline: schema.array(schema.string()),
    evolution: schema.union([schema.string(), schema.null()]),
    cultural: schema.union([schema.string(), schema.null()]),
    hasPlaceholders: schema.boolean(),
...
const setupSchema = schema
  .object({
    players: schema.string().min(5),
    deck: schema.string().min(5),
    equipment: schema.string(),
    dealing: schema.string().min(5),
    hasPlaceholders: schema.boolean(),
  })
  .strict();
...
const rulesSchema = schema
  .object({
    objective: schema.string().min(15),
    gameplay: schema.string().min(30),
    keyRules: schema.array(schema.string().min(5)).min(1),
    hasPlaceholders: schema.boolean(),
  })
  .strict();
...
const scoringSchema = schema
  .object({
    description: schema.string().min(5),
    winCondition: schema.string().min(5),
    cardValues: schema.record(schema.string()),
    penalties: schema.record(schema.string()),
    splitRules: schema.union([schema.string(), schema.null()]),
    targetScore: schema.union([
      schema.number(),
      schema.enum(TARGET_SCORE_NA_VALUES),
      schema.null(),
    ]),
    scoringDirection: schema.union([
      schema.enum(SCORING_DIRECTION_VALUES),
      schema.null(),
    ]),
    hasPlaceholders: schema.boolean(),
    nullReasons: nullReasonsSchema,
...
const strategySchema = schema
  .object({
    basic: schema.union([schema.string(), schema.null()]),
    intermediate: schema.union([schema.string(), schema.null()]),
    advanced: schema.union([schema.string(), schema.null()]),
...
const variationItemSchema = schema
  .object({
    id: schema.string(),
    name: schema.string(),
    description: schema.string(),
    ...
    emptyOverridesReason: schema.string().min(15).optional(),
  })
  .strict();

const variationsSchema = schema
  .object({
    list: schema.array(variationItemSchema),
    hasPlaceholders: schema.boolean(),
    noVariationsReason: schema.string().min(15).optional(),
  })
  .strict()
  .refine(
...
const aiSchema = schema
  .object({
    difficulty: schema
      .object({
        easy: schema.union([schema.string(), schema.null()]),
        medium: schema.union([schema.string(), schema.null()]),
        hard: schema.union([schema.string(), schema.null()]),
      })
      .strict(),
    considerations: schema.array(schema.string()),
    hasPlaceholders: schema.boolean(),
    nullReasons: nullReasonsSchema,
  })
  .strict()
...
const sourceSectionSchema = schema
  .object({
    section: schema.string().optional(),
    sourcePath: schema.string().optional(),
    paragraph: schema.number().optional(),
  })
  .strict();

const primarySourceSchema = schema
  .object({
    id: schema.union([
      schema.string().regex(/^src:[a-z0-9_-]+$/),
      schema.enum(NA_UNKNOWN_VALUES),
    ]),
    name: schema.string(),
    url: schema.string().url(),
    retrievedAt: schema.string().optional(),
    sections: schema.array(sourceSectionSchema).optional(),
    localHtml: schema.string().optional(),
  })
  .strict();

const sourcesSchema = schema
  .object({
  primary: schema.array(primarySourceSchema).min(1),
  additional: schema.array(
    schema.union([
      schema.string(),
      schema.object({
...
const evidenceItemSchema = schema
  .object({
    ...
    quote: schema.string().optional(),
    confidence: schema.number().min(0).max(1).optional(),
    notes: schema.union([schema.string(), schema.null()]).optional(),
  })
  .strict();

const evidenceSchema = schema.array(evidenceItemSchema);

const overviewNaReasonsSchema = schema
  .object({
    category: schema.string().min(15).optional(),
    subCategory: schema.string().min(15).optional(),
    difficulty: schema.string().min(15).optional(),
    origin: schema.string().min(15).optional(),
    originName: schema.string().min(15).optional(),
    duration: schema.string().min(15).optional(),
  })
  .strict()
  .optional();
...
const extractionSchema = schema
  .object({
    status: schema.enum(EXTRACTION_STATUS_VALUES),
    missingCritical: schema.array(schema.string()).optional(),
    assumptions: schema.array(schema.string()).optional(),
    openQuestions: schema.array(schema.string()).optional(),
    validatorVersion: schema.string().optional(),
    overviewNaReasons: overviewNaReasonsSchema,
  })
  .strict();

const playerActionsSchema = schema
  .object(
    Object.fromEntries(ACTION_ID_VALUES.map((id) => [id, playerActionSchema]))
  )
  .strict();

const cardVisibilitySchema = schema
  .object({
  handDefault: schema.enum(HAND_DEFAULT_VALUES),
  initialDeal: schema.enum(INITIAL_DEAL_VALUES),
  marketCards: schema.union([
    schema.enum(MARKET_CARDS_VALUES),
    schema.null(),
  ]),
  stockPurchase: schema.union([
    schema.enum(STOCK_PURCHASE_VALUES),
    schema.null(),
  ]),
  discardTop: schema.union([
    schema.enum(DISCARD_TOP_VALUES),
    schema.null(),
  ]),
  tableauCards: schema.union([
    schema.enum(TABLEAU_CARDS_VALUES),
    schema.null(),
  ]),
  })
  .strict();
...
const zoneSchema = schema
  .object({
  id: zoneIdSchema,
  type: schema.enum(ZONE_TYPE_VALUES),
  visibility: schema.enum(ZONE_VISIBILITY_VALUES),
  owner: schema.enum(ZONE_OWNER_VALUES),
  capacity: schema.union([schema.number().int(), schema.null()]).optional(),
  })
  .strict();

const engineRulesSchema = schema
  .object({
    edgeCases: schema.array(
    schema
      .object({
        id: schema.string(),
        appliesTo: schema.string().optional(),
        text: schema.string(),
        affects: schema.string().optional(),
      })
      .strict()
  ),
  })
...
const drawSourceSchema = schema
  .object({
    source: schema.enum(DRAW_SOURCE_VALUES),
    visibility: schema.enum(DRAW_VISIBILITY_VALUES),
    isOptional: schema.boolean(),
    canPickAny: schema.boolean(),
    mustRevealCard: schema.union([schema.boolean(), schema.null()]),
    maxPerTurn: schema.union([schema.number().int(), schema.null()]),
    notes: schema.union([schema.string(), schema.null()]),
  })
  .strict();

const drawConfigSchema = schema
  .object({
    canDraw: schema.boolean(),
    drawSources: schema.array(drawSourceSchema),
    drawCount: schema.union([schema.number().int(), schema.null()]),
    drawTiming: schema.enum(DRAW_TIMING_VALUES),
    mustDrawBeforePlay: schema.boolean(),
    drawAndDiscard: schema.boolean(),
  })
  .nullable();

const discardConfigSchema = schema
  .object({
    hasDiscard: schema.boolean(),
    mustDiscard: schema.boolean(),
    discardTiming: schema.enum(DISCARD_TIMING_VALUES),
    discardVisibility: schema.enum(DISCARD_VISIBILITY_VALUES),
    discardCount: schema.union([schema.number().int(), schema.null()]),
    opponentCanPickFromDiscard: schema.boolean(),
    discardPickRules: schema
      .object({
        canPickTop: schema.boolean(),
        canPickAny: schema.boolean(),
        mustTakeAll: schema.boolean(),
        mustUsePickedCard: schema.boolean(),
        pickCost: schema.union([schema.string(), schema.null()]),
        frozenPileRules: schema.union([schema.string(), schema.null()]),
      })
      .nullable(),
  })
  .nullable();

const naUnknownNull = schema.union([
  schema.enum(NA_UNKNOWN_VALUES),
  schema.null(),
]);

const trumpConfigSchema = schema
  .object({
    hasTrump: schema.boolean(),
    trumpDetermination: schema.enum(TRUMP_DETERMINATION_VALUES),
    permanentTrumps: schema
      .array(
        schema.object({
          card: schema.string(),
          rank: schema.union([schema.number().int(), schema.null()]),
          name: schema.union([schema.string(), schema.null()]),
        })
      )
      .nullable(),
    rightBowerLeftBower: schema.boolean(),
    trumpSuitRanking: schema.union([schema.string(), schema.null()]),
    canCallNoTrump: schema.boolean(),
    overtakingRule: schema.union([schema.string(), schema.null()]),
  })
  .nullable();

const meldConfigSchema = schema
  .object({
    hasMelding: schema.boolean(),
    meldTypes: schema.array(
      schema.object({
        type: schema.enum(MELD_TYPE_VALUES),
        minSize: schema.number().int(),
        maxSize: schema.union([schema.number().int(), schema.null()]),
        acesHigh: schema.union([schema.boolean(), schema.null()]),
        wildcardAllowed: schema.boolean(),
      })
    ),
    meldTiming: schema.enum(MELD_TIMING_VALUES),
    layOffAllowed: schema.boolean(),
    initialMeldRequirement: schema.union([schema.string(), schema.null()]),
    goingOut: schema.object({
      condition: schema.string(),
      mustAnnounce: schema.boolean(),
      knockOption: schema.boolean(),
      knockDeadwoodLimit: schema.union([schema.number().int(), schema.null()]),
    }),
  })
  .nullable();

const trickConfigSchema = schema
  .object({
    hasTricks: schema.boolean(),
    tricksPerRound: schema.union([schema.number().int(), schema.null()]),
    mustFollowSuit: schema.boolean(),
    canTrumpFirstTrick: schema.boolean().optional(),
    mustOvertrump: schema.union([schema.boolean(), schema.null()]),
    leadRestrictions: schema.union([schema.string(), schema.null()]),
    trickWinCondition: schema.enum(TRICK_WIN_CONDITION_VALUES),
    trickWinnerLeads: schema.boolean(),
    scoredTricks: schema.boolean(),
    bidding: schema
      .object({
        hasBidding: schema.boolean(),
        biddingSystem: schema.enum(BIDDING_SYSTEM_VALUES),
        minBid: schema.union([schema.number().int(), schema.null()]),
        maxBid: schema.union([schema.number().int(), schema.null()]),
        passAllowed: schema.boolean(),
        doubling: schema.boolean(),
      })
      .nullable(),
    pointValues: schema
      .object({
        defaultPerSuit: schema.record(schema.string(), schema.number()).optional(),
        specificCards: schema
          .array(
            schema
              .object({
                card: schema.string(),
...
const declarationMechanismSchema = schema
  .object({
    type: schema.enum(DECLARATION_TYPE_VALUES),
    encoding: schema.record(schema.string(), schema.enum(DECLARATION_ENCODING_VALUES)),
    revealTiming: schema.enum(REVEAL_TIMING_VALUES),
    pigRule: schema.boolean(),
    pigPenalty: schema.union([schema.enum(PIG_PENALTY_VALUES), schema.null()]),
  })
  .nullable();

const handRanksSchema = schema
  .object({
    high: schema.union([schema.enum(HAND_RANK_HIGH_VALUES), schema.null()]),
    low: schema.union([schema.enum(HAND_RANK_LOW_VALUES), schema.null()]),
    lowQualifier: schema
      .object({
        maxHighCard: schema.number().int(),
        acePlaysLow: schema.boolean(),
        straightsAndFlushesCount: schema.boolean(),
      })
      .nullable(),
  })
  .nullable();

const buyCostsSchema = schema
  .object({
    enabled: schema.boolean(),
    currency: schema.enum(BUY_CURRENCY_VALUES),
    sources: schema.object({
      market: schema.object({
        byRank: schema.record(schema.string(), schema.number()),
        flat: schema.union([schema.number(), schema.null()]),
      }),
      stock: schema.object({ flat: schema.number() }),
      discard: schema.union([
        schema.object({ flat: schema.number() }),
        schema.enum(NA_UNKNOWN_VALUES),
        schema.null(),
      ]),
    }),
...
const marketConfigSchema = schema
  .object({
    enabled: schema.boolean(),
    size: schema.number().int(),
    refillFrom: schema.enum(REFILL_FROM_VALUES),
    refillTiming: schema.enum(REFILL_TIMING_VALUES),
    visibility: schema.enum(MARKET_VISIBILITY_VALUES),
  })
  .nullable();
...
const wildcardSchema = schema
  .object({
    card: schema.string(),
    canSubstituteFor: schema.enum(CAN_SUBSTITUTE_FOR_VALUES),
    restrictions: schema.union([schema.string(), schema.null()]),
    naturalPreferred: schema.boolean(),
  })
  .strict();

const actionCardSchema = schema
  .object({
    card: schema.string(),
    action: schema.enum(ACTION_CARD_ACTION_VALUES),
    drawCount: schema.union([schema.number().int(), schema.null()]),
    targetPlayer: schema.enum(TARGET_PLAYER_VALUES),
    notes: schema.union([schema.string(), schema.null()]),
  })
  .strict();

const bonusCardSchema = schema
  .object({
    card: schema.string(),
    bonusPoints: schema.number().int(),
    condition: schema.union([schema.string(), schema.null()]),
  })
  .strict();

const penaltyCardSchema = schema
  .object({
    card: schema.string(),
    penaltyPoints: schema.number().int(),
    notes: schema.union([schema.string(), schema.null()]),
  })
  .strict();

const specialCardsSchema = schema.union([
  schema.object({
    wildcards: schema.array(wildcardSchema).nullable(),
    actionCards: schema.array(actionCardSchema).nullable(),
    bonusCards: schema.array(bonusCardSchema).nullable(),
    penaltyCards: schema.array(penaltyCardSchema).nullable(),
  }),
  naUnknownNull,
]);

const sheddingSchema = schema.union([
  schema.object({
    hasShedding: schema.boolean(),
    sheddingGoal: schema.enum(SHEDDING_GOAL_VALUES),
    validPlays: schema.array(schema.object({ type: schema.enum(VALID_PLAY_TYPE_VALUES), notes: schema.union([schema.string(), schema.null()]) })),
    passAllowed: schema.boolean(),
    burnRules: schema.union([schema.string(), schema.null()]),
  }),
  naUnknownNull,
]);

const fishingConfigSchema = schema.union([
  schema.object({
    hasFishing: schema.boolean(),
    captureMethod: schema.enum(CAPTURE_METHOD_VALUES),
    captureTarget: schema.union([schema.number().int(), schema.null()]),
    sweepBonus: schema.union([schema.number().int(), schema.null()]),
    tableauStartSize: schema.union([schema.number().int(), schema.null()]),
  }),
  naUnknownNull,
]);

const patienceConfigSchema = schema.union([
  schema.object({
    isSolitaire: schema.boolean(),
    tableauColumns: schema.union([schema.number().int(), schema.null()]),
    foundationCount: schema.union([schema.number().int(), schema.null()]),
    buildDirection: schema.union([schema.enum(BUILD_DIRECTION_VALUES), schema.null()]),
    buildSuitRule: schema.union([schema.enum(BUILD_SUIT_RULE_VALUES), schema.null()]),
    redealAllowed: schema.boolean(),
    redealCount: schema.union([schema.number().int(), schema.null()]),
  }),
  naUnknownNull,
]);

const bankingConfigSchema = schema.union([
  schema.object({
    hasBanker: schema.boolean(),
    bankerDetermination: schema.enum(BANKER_DETERMINATION_VALUES),
    targetValue: schema.union([schema.number().int(), schema.null()]),
    bustRule: schema.union([schema.string(), schema.null()]),
    playerVsBanker: schema.boolean(),
  }),
  naUnknownNull,
]);

const playerConfigSchemaBase = schema
  .object({
  playerMode: schema.enum(PLAYER_MODE_VALUES),
  minPlayers: schema.number().int().min(1),
  maxPlayers: schema.number().int().min(1),
  optimalPlayers: schema.union([schema.number().int(), schema.null()]),
  startingStack: schema.union([schema.number().int(), schema.null()]),
  seatLayout: schema.enum(SEAT_LAYOUT_VALUES),
  partnerships: schema.boolean(),
  partnershipFormats: schema.array(schema.enum(PARTNERSHIP_FORMAT_VALUES)).optional(),
  })
  .strict();
...
const turnOrderSchema = schema
  .object({
  direction: schema.enum(TURN_DIRECTION_VALUES),
  startsWith: schema.enum(TURN_STARTS_WITH_VALUES),
  dealerRotates: schema.boolean(),
  })
  .strict();

const roundConfigSchema = schema.union([
  schema.object({
    hasRounds: schema.boolean(),
    roundCount: schema.union([schema.number().int(), schema.null()]),
    roundEndCondition: schema.enum(ROUND_END_CONDITION_VALUES),
    gameEndCondition: schema.enum(GAME_END_CONDITION_VALUES),
  }),
  naUnknownNull,
]);

const engineSchemaBase = schema
  .object({
  phases: schema.array(phaseSchema).min(1),
  ...
  initialHandSize: schema.number().int(),
  ...
  constants: schema.record(schema.enum(CONSTANTS_KEYS), schema.number()),
  finalHandSize: schema.union([schema.number().int(), schema.null()]),
  deckCount: schema.union([schema.number().int().min(1), schema.null()]),
  zones: schema.array(zoneSchema),
  rules: engineRulesSchema,
  implementationHints: schema
    .object({
      rngUsed: schema.array(schema.enum(RNG_USED_VALUES)).optional(),
      authoritativeServer: schema.boolean().optional(),
      customLogicNeeded: schema.array(schema.string()).optional(),
    })
    .strict(),
  progression: schema.array(schema.string()).optional(),
  roles: schema.array(schema.string().regex(/^[a-z][a-z0-9_]*$/)).optional(),
  customActions: schema.array(playerActionSchema.and(schema.object({ id: schema.string().regex(/^[a-z][a-z0-9_]*$/) }))).optional(),
  jokerCount: schema.number().int().min(0).optional(),
  deckDescription: schema
    .string()
    .min(1)
    .refine(
  ...
  rankDescription: schema
    ...
    .optional(),
  dealPattern: schema.string().optional(),
  bettingLimits: schema.enum(BETTING_LIMITS_VALUES).optional(),
  actions: schema.record(schema.unknown()).optional(),
  handRankingSystem: schema.union([schema.string(), schema.null()]).optional(),
  rankingSystem: schema.union([schema.string(), schema.null()]).optional(),
  useTrump: schema.boolean().optional(),
  notApplicableReasons: schema
    .record(
      schema.string(),
      schema
        .string()
...
const synthesisSchema = schema
  ...
  hero: schema.object({
    ...
    subtitle: schema.string().min(2),
    tagline: schema.string().min(5),
  }),
  shortDescription: schema.string().min(15),
  uiThemes: schema.array(schema.enum(UI_THEMES_VALUES)),
  uiLayout: schema.object({
    zones: schema.array(zoneIdSchema),
    marketPosition: schema.union([schema.enum(MARKET_POSITION_VALUES), schema.null()]),
    stockPosition: schema.union([schema.enum(STOCK_POSITION_VALUES), schema.null()]),
    discardPosition: schema.union([schema.enum(DISCARD_POSITION_VALUES), schema.null()]),
    playerHandLayout: schema.enum(PLAYER_HAND_LAYOUT_VALUES),
    potPosition: schema.union([schema.enum(POT_POSITION_VALUES), schema.null()]),
  }),
  })
  .strict();
...
interface IOverview extends schema.infer<typeof overviewSchema> {}
interface IHistory extends schema.infer<typeof historySchema> {}
interface ISetup extends schema.infer<typeof setupSchema> {}
interface IRules extends schema.infer<typeof rulesSchema> {}
interface IStrategy extends schema.infer<typeof strategySchema> {}
interface IVariations extends schema.infer<typeof variationsSchema> {}
interface IAi extends schema.infer<typeof aiSchema> {}
interface ISources extends schema.infer<typeof sourcesSchema> {}
interface IEngine extends schema.infer<typeof engineSchema> {}
interface ISynthesis extends schema.infer<typeof synthesisSchema> {}
interface IPrompts extends schema.infer<typeof promptsSchema> {}
interface IScoring extends schema.infer<typeof scoringSchema> {}
interface IEvidence extends schema.infer<typeof evidenceSchema> {}
interface IExtraction extends schema.infer<typeof extractionSchema> {}

const _GameSchemaInner = schema
  .object({
    schemaVersion: schema.string(),
    engineModelVersion: schema.string(),
    filename: schema.string(),
    name: schema.string(),
    completeness: completenessSchema,
    quality: schema.enum(QUALITY_VALUES),
    qualityReason: schema.string().optional(),
  ...
  tags: schema.array(schema.enum(TAGS_VALUES)),
  legal: schema
    .object({
      status: schema.enum(LEGAL_STATUS_VALUES),
      reasonForNA: schema.string().optional(),
      isCommercial: schema.boolean(),
      trademarkNote: schema.union([schema.string(), schema.null()]),
    })
    .strict(),
  statistics: schema
    .object({
      popularity: schema.number().int().min(1).max(100),
      complexity: schema.number().int().min(1).max(5),
      luck: schema.number().int().min(1).max(10),
      skill: schema.number().int().min(1).max(10),
    })
    .strict(),
  media: schema
    .object({
      videoTutorial: schema.union([schema.string(), schema.null()]),
    })
    .strict(),
    ...
    alsoKnownAs: schema.union([schema.array(schema.string()), schema.null()]),
    evidence: reference<IEvidence>(evidenceSchema),
    extraction: reference<IExtraction>(extractionSchema),
    fieldStatus: schema.record(schema.enum(FIELD_STATUS_VALUES)).optional(),
  })
  .strict()
  .refine(
    (g) => {
  ...
  .refine(
    (g) => {
      const ids = new Set(g.engine.zones.map((zone) => zone.id));
  ...
  .refine(
    (g) => {
      ...
      const zoneIds = new Set(g.engine.zones.map((zone) => zone.id));
...
export type Game = schema.output<typeof _GameSchemaInner>;
