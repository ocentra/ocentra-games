...
export class MainAppLogger extends BaseLogger {
  private static _instance: MainAppLogger | null = null;
  ...
  static get instance(): MainAppLogger {
    if (!MainAppLogger._instance) {
      const pathResolver = MainAppLogger._storedPathResolver ?? {
        getFilePathFromUrl: (url: string) => url,
        getSourceFromFilePath: (_filePath?: string) => 'unknown',
      };
      MainAppLogger._instance = new MainAppLogger(
        MainAppLogger._storedStorage ?? null,
        pathResolver,
        MainAppLogger._storedConfig,
        MainAppLogger._storedRequestContextProvider
      );
    }
    return MainAppLogger._instance;
  }
  ...
  static initLogger(
    ...
    MainAppLogger._storedStorage = storage;
    MainAppLogger._storedPathResolver = pathResolver;
    MainAppLogger._storedRequestContextProvider = requestContextProvider ?? null;
    MainAppLogger._storedConfig = config;
    ...
    if (!MainAppLogger._instance) {
      ...
      MainAppLogger._instance.updateConfig(config || {});
      MainAppLogger._instance._requestContextProvider = MainAppLogger._storedRequestContextProvider;
      if (storage !== MainAppLogger._instance.storage) {
        MainAppLogger._instance.storage = storage;
      }
    }
    return MainAppLogger._instance;
  }

  static create(storage: ILogStorage | null, pathResolver: PathResolver, config?: LoggerConfig): MainAppLogger {
    return MainAppLogger.initLogger(storage, pathResolver, config);
  }

  static configure(storage: ILogStorage | null, pathResolver: PathResolver, config: LoggerConfig): MainAppLogger {
    return MainAppLogger.initLogger(storage, pathResolver, config);
  }

  override register(urlOrName: string | undefined, urlOrOptions?: string | { batch?: boolean }): void {
    super.register(urlOrName, urlOrOptions);
  }
  ...
  protected override getLogSource(registeredUser: RegistrationInfo | null, primaryFrame: StackFrame | null): LogSource {
    if (registeredUser) {
      return `${LogSourcePrefix.Browser}${registeredUser.className}`;
    }
    return this.pathResolver.getSourceFromFilePath(primaryFrame?.filePath);
  }

  protected override writeToConsole(payload: StructuredLogPayload): void {
  ...
  protected override async storeLogEntry(
    level: LogLevel,
    source: LogSource,
    context: string,
    message: string,
  ...
  static override reset(): void {
    BaseLogger.reset(MainAppLogger._instance || undefined);
    MainAppLogger._instance = null;
    MainAppLogger._storedStorage = null;
    MainAppLogger._storedPathResolver = null;
    MainAppLogger._storedRequestContextProvider = null;
    MainAppLogger._storedConfig = undefined;
  }
}
