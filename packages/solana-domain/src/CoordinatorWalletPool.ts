...
export class CoordinatorWalletPool {
  ...
  async rotateWallet(): Promise<void> {
    ...
    logInfo(`Wallet rotated: ${previousIndex} -> ${this.currentIndex} (pubkey: ${this.hotWallets[this.currentIndex].publicKey.toBase58()})`);

    if (this.metricsCollector) {
      logInfo(`Wallet rotation recorded at transaction count: ${this.rotationThreshold}`);
    }
  }
