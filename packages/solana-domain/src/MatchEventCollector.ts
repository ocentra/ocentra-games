...
import type { MatchState } from './GameClient';
import { PublicKey } from '@solana/web3.js';
import type { MatchRecord, MoveRecord, PlayerRecord } from '@ocentra/verification-domain/types';
...
const logWarn = (message: string, dataOrEnabled?: unknown | boolean, enabled: boolean = LOG_NETWORK) => {
...
export class MatchEventCollector {
  ...
  private async collectMoves(matchId: string, matchState: MatchState): Promise<MoveRecord[]> {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        while (hasMore) {
          const filters: Array<{ memcmp?: { offset: number; bytes: Uint8Array }; dataSize?: number }> = [
            {
              memcmp: {
                offset: 8,  // Skip discriminator (8 bytes) - per critique: validate offset
                bytes: new TextEncoder().encode(matchId),
              },
            },
          ];
          ...
          try {
            ...
            logWarn('program.account.move not available, using simplified move collection', undefined, LOG_NETWORK);
            hasMore = false;
            break;
          }
          ...
          if (allMoveAccounts.length > matchState.moveCount * 2) {
            logWarn('Collected more moves than expected, stopping pagination', undefined, LOG_NETWORK);
            hasMore = false;
          }
        }
        ...
        logError(`Error collecting moves (attempt ${attempt + 1}/${maxRetries}):`, lastError, LOG_NETWORK);
  ...
  private getActionTypeName(actionType: number): string {
    const mapping: Record<number, string> = {
      0: PlayerActionType.PICK_UP,
      1: PlayerActionType.DECLINE,
      2: PlayerActionType.DECLARE_INTENT,
      3: PlayerActionType.CALL_SHOWDOWN,
      4: PlayerActionType.REBUTTAL,
    };
   ...
   */
  private deserializePayload(payload: number[] | Uint8Array): unknown {
    try {
      ...
      try {
          ...
          logError('Error parsing JSON payload:', parseError, LOG_NETWORK);
        }
      }

      const asArray = Array.from(bytes);

      if (asArray.length === 2 && asArray[0] < 4 && asArray[1] >= 2 && asArray[1] <= 14) {
        return { suit: asArray[0], value: asArray[1] };
      } else if (asArray.length === 6 && asArray.length % 2 === 0) {
        const cards = [];
        for (let i = 0; i < asArray.length; i += 2) {
          if (asArray[i] < 4 && asArray[i + 1] >= 2 && asArray[i + 1] <= 14) {
            cards.push({ suit: asArray[i], value: asArray[i + 1] });
          }
        }
        if (cards.length > 0) {
          return { cards };
      ...
      logError('Error deserializing payload:', error, LOG_NETWORK);
      const bytes = payload instanceof Uint8Array ? payload : new Uint8Array(payload);
