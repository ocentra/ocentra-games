...
export class DifferentColorsSequence extends BaseBonusRule {
    @serializable({ label: 'Minimum Cards Required' })
  override minNumberOfCard: number = 3;

  @serializable({ label: 'Default Bonus Value' })
  override bonusValue: number = 115;

  @serializable({ label: 'Pattern Type' })
  override patternType: string = 'different_colors_sequence';

  @serializable({ label: 'Rule Name' })
  override ruleName: string = 'DifferentColorsSequence';

  @serializable({ label: 'Priority' })
  override priority: number = 89;
  ...
  async evaluate(
    ...
    const descriptions: string[] = ['Different Colors Sequence'];
