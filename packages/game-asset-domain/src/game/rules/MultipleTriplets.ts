...
export class MultipleTriplets extends BaseBonusRule {
    @serializable({ label: 'Minimum Cards Required' })
  override minNumberOfCard: number = 6;

  @serializable({ label: 'Default Bonus Value' })
  override bonusValue: number = 150;

  @serializable({ label: 'Pattern Type' })
  override patternType: string = 'multiple_triplets';

  @serializable({ label: 'Rule Name' })
  override ruleName: string = 'MultipleTriplets';

  @serializable({ label: 'Priority' })
  override priority: number = 96;
  ...
  async evaluate(hand: Card[], trumpCard?: Card): Promise<BonusDetail | null> {
    ...
    const descriptions: string[] = multipleTriplets.map(rank => 
      `Three of a Kind: ${cardRanking.getRankName(rank)}`
    );
