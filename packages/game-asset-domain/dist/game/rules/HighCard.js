...
let HighCard = class HighCard extends BaseBonusRule {
    minNumberOfCard = 3;
    bonusValue = 90;
    patternType = 'high_card';
    ruleName = 'HighCard';
    priority = 86;
    async initialize(gameMode) {
        ...
        this.description = 'Highest value card in hand when no other hands are made.';
    ...
    async evaluate(hand, _trumpCard) {
        ...
        const highestCard = hand.find(c => c.value === highestValue);
        const cardRanking = this.gameMode ? await this.gameMode.getCardRanking() : null;
        if (!cardRanking) {
            return null;
        }
        const rankName = cardRanking.getRankName(highestValue);
        const descriptions = [`High Card: ${rankName}`];
        return new BonusDetail(this.ruleName, baseBonus, 0, descriptions, calculation, this.priority, highestCard ? [highestCard] : []);
    }
...
], HighCard.prototype, "minNumberOfCard", void 0);
__decorate([
    serializable({ label: 'Default Bonus Value' }),
    __metadata("design:type", Number)
], HighCard.prototype, "bonusValue", void 0);
__decorate([
    serializable({ label: 'Pattern Type' }),
    __metadata("design:type", String)
], HighCard.prototype, "patternType", void 0);
__decorate([
    serializable({ label: 'Rule Name' }),
    __metadata("design:type", String)
], HighCard.prototype, "ruleName", void 0);
__decorate([
    serializable({ label: 'Priority' }),
    __metadata("design:type", Number)
], HighCard.prototype, "priority", void 0);
HighCard = __decorate([
    serializableClass({
        assetType: 'HighCard',
