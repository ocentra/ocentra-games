...
let DifferentColorsSequence = class DifferentColorsSequence extends BaseBonusRule {
    minNumberOfCard = 3;
    bonusValue = 115;
    patternType = 'different_colors_sequence';
    ruleName = 'DifferentColorsSequence';
    priority = 89;
    async initialize(gameMode) {
        ...
        this.description = 'Sequence of cards with different colors.';
    ...
    async evaluate(hand, _trumpCard) {
        ...
        if (!HandUtility.isDifferentColorsSequence(hand)) {
            return null;
        }
        ...
        const descriptions = ['Different Colors Sequence'];
    ...
    createExampleHand(handSize, cardRanking, _trumpCard, coloured = true) {
        ...
        const hand = [];
        const allSuits = cardRanking.getAllSuits();
        const startRank = cardRanking.getRandomRank(2, 14 - handSize + 1);
        for (let i = 0; i < handSize; i++) {
            const rank = startRank + i;
            const suit = allSuits[i % allSuits.length];
...
], DifferentColorsSequence.prototype, "minNumberOfCard", void 0);
__decorate([
    serializable({ label: 'Default Bonus Value' }),
    __metadata("design:type", Number)
], DifferentColorsSequence.prototype, "bonusValue", void 0);
__decorate([
    serializable({ label: 'Pattern Type' }),
    __metadata("design:type", String)
], DifferentColorsSequence.prototype, "patternType", void 0);
__decorate([
    serializable({ label: 'Rule Name' }),
    __metadata("design:type", String)
], DifferentColorsSequence.prototype, "ruleName", void 0);
__decorate([
    serializable({ label: 'Priority' }),
    __metadata("design:type", Number)
], DifferentColorsSequence.prototype, "priority", void 0);
DifferentColorsSequence = __decorate([
    serializableClass({
...
export { DifferentColorsSequence };
