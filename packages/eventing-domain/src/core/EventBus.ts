...
export class EventBus implements IEventBus {
  ...
  private readonly logWarning = (message: string, data?: unknown) => {
    this.log.logWarn(message, getStackTrace(), data);
  };

  constructor(options: EventBusOptions = DEFAULT_OPTIONS) {
  ...
  async publish<T extends IEventArgs>(
    eventArgs: T,
    options: { awaitAsync?: boolean; force?: boolean; timeoutMs?: number } = {}
  ): Promise<OperationResult<boolean>> {
    const { awaitAsync = false, force = false, timeoutMs } = options;
    try {
      const handled = await this.guardAndPublish(eventArgs, awaitAsync, force, true, timeoutMs);
      return OperationResult.success(handled);
    } catch (error) {
      const message =
        error instanceof Error ? error.message : `Failed to publish ${eventArgs.constructor.name}`;
      this.logError(`Error publishing event ${eventArgs.constructor.name}`, error);
  ...
  publishAsync<T extends IEventArgs>(
    ...
    const publishPromise = this.publish(eventArgs, { awaitAsync: true, force: options.force ?? false, timeoutMs: options.timeoutMs });
  ...
  private async guardAndPublish<T extends IEventArgs>(
    ...
    try {
      const handled = await this.handlePublishAction(eventArgs, awaitAsyncSubscribers, timeoutMs);
      ...
      if (!handled && allowQueue) {
        ...
        if (queued) {
          ...
          eventArgs.dispose();
          this.logError(`Failed to queue event of type ${eventType}.`);
        }
      } else {
        eventArgs.dispose();
      }
      ...
      this.logError(`Error in publish for event type ${eventType}`, error);
      throw error;
    } finally {
  ...
  private async handlePublishAction<T extends IEventArgs>(
    ...
    if (asyncSubscribers?.length) {
      ...
      if (awaitAsyncSubscribers) {
        const subscriberTimeout = timeoutMs && timeoutMs > 0 ? timeoutMs : undefined;
        const subscriberPromises = [...asyncSubscribers].map(subscriber =>
          this.invokeAsyncSubscriber(subscriber as AsyncHandler<T>, eventArgs, key, subscriberTimeout !== undefined, subscriberTimeout)
        );
        const results = await Promise.allSettled(subscriberPromises);
        for (const result of results) {
          if (result.status === 'rejected') {
            this.logError(
              `Async subscriber failed for event type ${key}: ${result.reason instanceof Error ? result.reason.message : String(result.reason)}`,
              result.reason
            );
          }
        }
  ...
  private async invokeAsyncSubscriber<T extends IEventArgs>(
    ...
    try {
      ...
      if (enforceTimeout && timeoutMs && timeoutMs > 0) {
        await withTimeout(
          result,
          timeoutMs,
      ...
      while (queue.length) {
        ...
        for (const queued of batch) {
          if (this.isExpired(queued)) {
            ...
            this.logWarning(`Dropping expired event of type ${key} from queue.`);
            continue;
          }

          try {
            const handled = await withTimeout(
              this.guardAndPublish(queued.event as T, true, true, false, this.options.asyncTimeoutMs),
  ...
  private requeueIfPossible(
    ...
    if (nextAttempt <= this.options.maxRetryAttempts) {
      ...
      this.logWarning(`Event ${key} has exceeded maximum retry attempts and will be dropped.`);
    }
  }
