...
describe(extractName(import.meta.url), TestSuiteType.Contract, () => {
  const provider = new PactV4({
    consumer: 'Frontend-Reward',
    provider: 'Cloudflare Worker',
    dir: pactDir,
    logLevel: 'info',
  });

  it('rewards daily: returns 200 with available and claimed for authenticated user', async () => {
    const pathSegment = ApiEndpoint.Rewards.Daily;

    await provider
      .addInteraction()
      .given('user has daily reward state')
      .uponReceiving('a request for daily reward')
      ...
      .willRespondWith(HttpStatus.Ok, (builder) => {
        ...
        builder.jsonBody(
          Matchers.like({
            ...
            claimed: Matchers.boolean(false),
          })
        );
      })
  ...
  it('rewards daily: returns 401 when authentication is missing', async () => {
    const pathSegment = ApiEndpoint.Rewards.Daily;

    await provider
      .addInteraction()
      .given('no authentication')
      .uponReceiving('a request for daily reward without auth')
      .withRequest(HttpMethod.Get, pathSegment, (builder) => {
        builder.headers({
          [HttpHeader.Origin]: TestConfig.LocalhostOrigin,
        });
