...
describe(extractName(import.meta.url), TestSuiteType.Contract, () => {
  const provider = new PactV4({
    consumer: 'Frontend-Matchmaking',
    provider: 'Cloudflare Worker',
    dir: pactDir,
    logLevel: 'info',
  });

  it('queue POST: returns 200 with ticketId or position', async () => {
    const pathSegment = ApiEndpoint.Matchmaking.Queue;

    await provider
      .addInteraction()
      .given('matchmaking queue accepts request')
      .uponReceiving('a request to join matchmaking queue')
      ...
      .willRespondWith(HttpStatus.Ok, (builder) => {
        ...
        builder.jsonBody(
          Matchers.like({
            ticketId: Matchers.string('ticket-uuid'),
            position: Matchers.integer(0),
          })
        );
      })
