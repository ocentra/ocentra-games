...
describe(extractName(import.meta.url), TestSuiteType.Contract, () => {
  const provider = new PactV4({
    consumer: 'Frontend-Settings',
    provider: 'Cloudflare Worker',
    dir: pactDir,
    logLevel: 'info',
  });

  it('settings by user: returns 200 with settings object for authenticated user', async () => {
    const userId = TestConfig.TestUserId;
    const pathSegment = ApiEndpoint.Settings.ByUser(userId);

    await provider
      .addInteraction()
      .given('user has settings')
      .uponReceiving('a request for settings by user')
  ...
  it('settings: returns 401 when authentication is missing', async () => {
    const pathSegment = ApiEndpoint.Settings.ByUser(TestConfig.TestUserId);

    await provider
      .addInteraction()
      .given('no authentication')
      .uponReceiving('a request for settings without auth')
      .withRequest(HttpMethod.Get, pathSegment, (builder) => {
        builder.headers({
          [HttpHeader.Origin]: TestConfig.LocalhostOrigin,
        });
