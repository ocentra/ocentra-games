...
async function expectForbiddenResponse(response: Response, expectedMessage: string): Promise<void> {
...
describe(extractName(import.meta.url), TestSuiteType.Integration, () => {
  ...
  it(testName('Progression POST xp: rejects client-authoritative XP'), async () => {
    const token = getTokenForFetch();
    ...
    await expectForbiddenResponse(response, 'Progression XP must be issued by trusted server workflows');
  });

  it(testName('Progression POST xp with idempotencyKey: rejects client-authoritative replay attempts'), async () => {
    const token = getTokenForFetch();
    const url = buildApiUrl(ApiEndpoint.Progression.Xp, { baseUrl: TestConfig.TestApiUrlPlaceholder });
    const key = `xp-client-mutation-${Date.now()}`;
    const body = JSON.stringify({ amount: 1000, idempotencyKey: key });
    const headers = {
      ...getValidRequestHeaders(TestConfig.TestUserId),
      [HttpHeader.ContentType]: HttpContentType.ApplicationJson,
    };
    const first = await worker.fetch(url, { method: HttpMethod.Post, headers, body }, token);
    await expectForbiddenResponse(first, 'Progression XP must be issued by trusted server workflows');
    const second = await worker.fetch(url, { method: HttpMethod.Post, headers, body }, token);
    await expectForbiddenResponse(second, 'Progression XP must be issued by trusted server workflows');
  });
  ...
  it(testName('Missions POST progress: rejects client-authoritative progress increments'), async () => {
    const token = getTokenForFetch();
    ...
    await expectForbiddenResponse(res, 'Mission progress must be issued by trusted server workflows');
  });

  it(testName('Missions POST claim: does not complete after rejected client-authoritative progress'), async () => {
    const token = getTokenForFetch();
    ...
    await expectForbiddenResponse(progressRes, 'Mission progress must be issued by trusted server workflows');
  ...
  it(testName('Missions POST claim: repeated claims remain blocked after rejected client progress'), async () => {
    const token = getTokenForFetch();
    ...
    expect(claimFirst.status).toBe(HttpStatus.BadRequest);
    await claimFirst.text().catch(() => undefined);
    const second = await worker.fetch(claimUrl, { method: HttpMethod.Post, headers, body }, token);
    expect(second.status).toBe(HttpStatus.BadRequest);
    const data = (await second.json()) as { error?: string; progress?: number };
    expect(typeof data.error).toBe('string');
    expect(data.progress).toBe(0);
  });
  ...
  it(testName('Progression POST xp: rejects missing or invalid client-authoritative XP before mutation'), async () => {
    const token = getTokenForFetch();
    const url = buildApiUrl(ApiEndpoint.Progression.Xp, { baseUrl: TestConfig.TestApiUrlPlaceholder });
    const headers = {
      ...getValidRequestHeaders(TestConfig.TestUserId),
    ...
    await expectForbiddenResponse(badBody, 'Progression XP must be issued by trusted server workflows');
    const negBody = await worker.fetch(url, { method: HttpMethod.Post, headers, body: JSON.stringify({ amount: -1 }) }, token);
    await expectForbiddenResponse(negBody, 'Progression XP must be issued by trusted server workflows');
  });

  it(testName('Progression: rejected client-authoritative XP does not grow levelHistory'), async () => {
    const token = getTokenForFetch();
    ...
    await expectForbiddenResponse(xpRes, 'Progression XP must be issued by trusted server workflows');
    ...
    expect(afterData.level).toBe(beforeData.level);
    expect(afterData.levelHistory?.length ?? 0).toBe(historyLenBefore);
  });
  ...
  it(testName('Missions POST progress: rejects client-authoritative unknown mission progress'), async () => {
    const token = getTokenForFetch();
    const progressUrl = `${TestConfig.TestApiUrlPlaceholder}${ApiEndpoint.Missions.Base}/progress`;
    ...
    await expectForbiddenResponse(response, 'Mission progress must be issued by trusted server workflows');
  });

  it(testName('Missions POST progress: rejects client-authoritative progress set'), async () => {
    const token = getTokenForFetch();
    const progressUrl = `${TestConfig.TestApiUrlPlaceholder}${ApiEndpoint.Missions.Base}/progress`;
    const headers = {
      ...getValidRequestHeaders(TestConfig.TestUserId),
  ...
  it(testName('Concurrency Rule 16.2.1: rejected client mission progress cannot seed concurrent rewards'), async () => {
    const token = getTokenForFetch();
    ...
    expect(responses.every((response) => response.status === HttpStatus.BadRequest)).toBe(true);
    const datas = await Promise.all(responses.map((r) => r.json() as Promise<{ claimed?: boolean; alreadyClaimed?: boolean }>));
    const firstClaimCount = datas.filter((d) => d.claimed && !d.alreadyClaimed).length;
    expect(firstClaimCount).toBe(0);
  });
  ...
  it(testName('Battle pass POST xp: rejects client-authoritative XP'), async () => {
    const token = getTokenForFetch();
    ...
    await expectForbiddenResponse(response, 'Battle pass XP must be issued by trusted server workflows');
  });
  ...
  it(testName('Progression POST unlock-skill: returns 400 when skill points are not server-earned'), async () => {
    const token = getTokenForFetch();
    const userId = `unlock-no-xp-${Date.now()}`;
    const headers = {
      ...getValidRequestHeaders(userId),
      [HttpHeader.ContentType]: HttpContentType.ApplicationJson,
    };
  ...
  it(testName('Progression POST unlock-skill: remains locked after rejected client-authoritative XP'), async () => {
    const token = getTokenForFetch();
    const userId = `unlock-rejected-xp-${Date.now()}`;
    const headers = {
      ...getValidRequestHeaders(userId),
      [HttpHeader.ContentType]: HttpContentType.ApplicationJson,
    };
    const getUrl = buildApiUrl(ApiEndpoint.Progression.Base, { baseUrl: TestConfig.TestApiUrlPlaceholder });
    const prog = await worker.fetch(getUrl, { method: HttpMethod.Get, headers: getValidRequestHeaders(userId) }, token);
    const progData = (await prog.json()) as { skillPoints?: number; xpToNextLevel?: number };
    const xpUrl = buildApiUrl(ApiEndpoint.Progression.Xp, { baseUrl: TestConfig.TestApiUrlPlaceholder });
    const xpRes = await worker.fetch(xpUrl, { method: HttpMethod.Post, headers, body: JSON.stringify({ amount: progData.xpToNextLevel ?? 1000 }) }, token);
    await expectForbiddenResponse(xpRes, 'Progression XP must be issued by trusted server workflows');
    const url = buildApiUrl(ApiEndpoint.Progression.UnlockSkill, { baseUrl: TestConfig.TestApiUrlPlaceholder });
    const skillId = `idem-skill-${Date.now()}`;
    const response = await worker.fetch(url, { method: HttpMethod.Post, headers, body: JSON.stringify({ skillId }) }, token);
    expect(response.status).toBe(HttpStatus.BadRequest);
    const data = (await response.json()) as { error?: string };
    expect(typeof data.error).toBe('string');
  });
  ...
  it(testName('Progression POST update-achievement: rejects client-authoritative achievement progress'), async () => {
    const token = getTokenForFetch();
    ...
    await expectForbiddenResponse(response, 'Achievement progress must be issued by trusted server workflows');
  });

  it(testName('Progression POST update-achievement: rejects malformed client-authoritative progress before mutation'), async () => {
    const token = getTokenForFetch();
    const url = buildApiUrl(ApiEndpoint.Progression.UpdateAchievement, { baseUrl: TestConfig.TestApiUrlPlaceholder });
    const response = await worker.fetch(url, {
      method: HttpMethod.Post,
