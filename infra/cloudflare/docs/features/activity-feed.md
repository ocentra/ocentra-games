...
**Purpose:** Per-user activity feed: append items, list with cursor. Fan-out: POST /fanout writes one item to each friend’s ActivityFeedDO (handler fetches friends from PresenceDO, then POSTs to each friend’s feed).
...
- POST /fanout: body type, payload; handler gets friends from PresenceDO, then for each friendId calls ActivityFeedDOPaths.Append on that friend’s DO with { type, payload: { ...payload, actorId } }; returns { fanout: appended, friends: friendIds.length }.
